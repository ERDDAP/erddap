 ...
 *
 * <UL>
 *   <LI>It is best to pass the programming form of the classAndMethodName, e.g., "ArrayInt.set".
 *   <LI>It is best to pass the menu form of the valueName, i.e., with capital first letters and
 *       spaces between words, e.g., "How Many".
 *   <LI>All of the error messages have short lines, suitable for a small dialog box (not a super
 *       wide, 1-line box).
 *   <LI>All of the error messages start with the classAndMethodName. [If JITs or compiling to
 *       native code may make the stack trace unavailable, it would be possible to make my own stack
 *       trace system by putting classAndMethodName at the end. Every routine passing on the error
 *       message could return error+"CurrentClass.method"+lineSeparator.]
 *   <LI>Although it makes sense to have these routines to check for the error and then return "" or
 *       an error message, it is clumsy to implement. It is better to do the exact check there, then
 *       generate this error message if there is an error. Also, some checks are on 0..n-1 versions
 *       of the value, but messages should be 1..n.
 *   <LI>Because the tests are done locally, there is no need to differentiate between different
 *       numeric data types; just use doubles here.
 *   <LI>Because the tests are done locally, there is no need to distinguish between &gt; and &gt;=,
 *       or between &lt; and &lt;=; both can use the same error messages.
 *   <LI>******* Don't generate lots of MustBe messages. throwableStackTrace's
 *       throwable.printStackTrace is very slow (~50ms) so all MustBe methods are pretty slow.
 * </UL>
 */
public class MustBe {
   ...
   * final so EDStatic can change it. (I'm not sure if that is a good idea.)
   */
  ...
  /**
   * This gets the current stack trace, which can be useful for debugging. This is useful for
  ...
  /**
   * Given a throwable, this generates a multiline string (with lineSeparator's) with the method
   * stack trace, e.g.,
   *
   * <pre>
   ...
   * @param nRemoveLines determines how many lines should be removed from the start of the stack
   *     trace. For handler routine and artificial Throwable (like MustBe.between -&lt;
   *     stackTrace()), 3 is recommended. For any routine handling a real Thrown exception (e.g.,
   *     MustBe.throwable), 0 is recommended.
   ...
   */
  public static String throwableStackTrace(Throwable throwable, int nRemoveLines
  ...
  /** Returns a string with t.toString but with "at java" and "at sun" lines removed. */
  ...
  public static String throwableToShortString(Throwable t) {
    ...
    if (atPo > 0) {
      int nPo = s.indexOf("\n", atPo + 6);
      if (nPo > 0) s = s.substring(0, nPo);
    }
    return s;
  }

  /**
   * This generates a method stack trace for the current position in the program.
   *
   * <UL>
   *   <LI>This creates an Exception object and then calls throwableStackTrace().
   * </UL>
   */
   ...
   *
   * <UL>
   *   <LI>This creates a Exception object and then calls throwableStackTrace().
   * </UL>
   */
   ...
   * <UL>
   *   <LI>E.g., MustBe.throwable("MyClass.myMethod", throwable);
   *   <LI>OutOfMemoryError is handled as a special case that prints a more extensive error message.
   *       In general, outOfMemory possibility can't just be ignored and caught. Methods need to try
   *       to catch it so that method can recover nicely.
   * </UL>
   *
   * @param classAndMethodName e.g., DataRound.testRound
   * @param message e.g., ERROR: Null
   */
  public static String throwableWithMessage(
      String classAndMethodName, String message, Throwable throwable) {
   ...
   * @param hideTomcatWaitingThreads if true, this tries to not show details of tomcat threads that
   *     are waiting (not really active), including inotify threads, and just counts them (just
   *     because there are so many of them and they aren't interesting).
   */
      ...
      for (int i = 0; i < oar.length; i++) {
        try {
          ...
          if (hideTomcatWaitingThreads
              &&
              // ste.length >= 3 &&
              t.getState().toString().endsWith("WAITING")
              && // WAITING and TIMED_WAITING
              (ste0.endsWith("jdk.internal.misc.Unsafe.park(Native Method)")
                  || // Java 17+
      ...
      sb.append(
          "Number of threads: "
              ...
              (hideTomcatWaitingThreads
                  ? "Tomcat-waiting=" + tomcatWaiting + ", inotify=" + inotify + ", other="
                  : "")
              + count
              + "\n"
              + "(format: #threadNumber Thread[threadName,threadPriority,threadGroup] threadStatus)\n\n");
      for (int i = 0; i < count; i++) sb.append("#" + (i + 1) + " " + sar[i]);
  ...
  public static String getShortErrorMessage(Throwable t) {
    ...
    if (tError.length() > 0
      ...
      if (cpo > 0) {
        ...
        if (ppo > 0) tError = tError.substring(ppo + 1);
