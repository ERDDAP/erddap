...
public enum PAType {
    ...
    /** This returns true for UBYTe, USHORT, UINT, and ULONG. */
    public final static boolean isUnsigned(PAType tPaType) {
    ...
    public final static boolean isIntegerType(PAType tPaType) {
        return switch (tPaType) {
    ...
    public final static PAType makeUnsigned(PAType tPaType) {
        return switch (tPaType) {
            case BYTE  -> UBYTE;
            case SHORT -> USHORT;
            case INT   -> UINT;
    ...
    public final static PAType makeSigned(PAType tPaType) {
        return switch (tPaType) {
            case UBYTE  -> BYTE;
            case USHORT -> SHORT;
            case UINT   -> INT;
    ...
    public static PAType fromCohortStringCaseInsensitive(String type) {  //was PrimitiveArray.caseInsensitiveElementStringToPAType(
        return switch (type.toLowerCase()) {
            ...
            case "ushort" -> USHORT;
            case "byte", 
                 "boolean"-> BYTE; //erddap stores booleans as bytes
            case "ubyte"  -> UBYTE;
            case "char"   -> CHAR;
            case "string" -> STRING;
            default -> {throw new IllegalArgumentException("PAType.fromCohortStringCaseInsensitive unsupported type: " + type); }
        };
    }
    ...
    public static int elementSize(PAType type) {
        return switch (type) {
            ...
            case BYTE,
                 UBYTE,
                 BOOLEAN-> 1; //erddap stores booleans as bytes
            case STRING -> 20;
            default -> {throw new IllegalArgumentException("PrimitiveArray.sizeOf unsupported type: " + type);}
        };
    }
