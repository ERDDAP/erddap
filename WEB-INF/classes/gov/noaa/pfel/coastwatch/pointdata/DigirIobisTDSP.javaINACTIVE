 ...
 */
public class DigirIobisTDSP implements thredds.servlet.DatasetSource {
    ...
    public boolean isMine(HttpServletRequest request) {
       ...
       return path.startsWith("/thredds/iobis/");
    }
    ...
    public NetcdfFile getNetcdfFile(HttpServletRequest request, 
        ...
        try {
            //parse the request
            String path = request.getPathInfo();
            path = path.substring("/thredds/iobis/".length());
String junk="";
            String query = junk;
            String resource = junk;
String2.log("  path=" + path);
String2.log("  resource=" + resource);
String2.log("  query=" + query);

            //get the data
            Table table = new Table();
            DigirHelper.searchObisOpendapStyle(new String[]{resource}, 
                Table.IOBIS_URL,  
                query, table);

            //save the data in a file
            File tFile = File.createTempFile("Iobis", ".nc");
            tFile.deleteOnExit();
