...
public class NdbcMetStation  {
     ...
     */
    public static String firstNearRealTimeData = "2013-02-01T00:00:00";
...
<img src="/icons/text.gif" alt="[TXT]" /> <a href="42OTP.txt">42OTP.txt</a>               28-Aug-2005 14:23   24K  Standard Meteorological Data
    ...
    public static void test46088(Table table) throws Exception {
        ...
        //2013 02 01 01 20 230  3.0  3.0   0.2    MM   3.8  MM 1025.3   8.0   7.6   7.5   MM   MM    MM
        //2013 02 01 00 50 180  2.0  3.0   0.2    MM   3.7  MM 1025.2   8.1   7.6   7.5   MM +0.0    MM
        seconds = Calendar2.isoStringToEpochSeconds("2013-02-01T01"); //50 min rounds to next hour
        row = table.getColumn(timeIndex).indexOf("" + seconds, 0);
        Test.ensureEqual(table.getStringData(idIndex, row), "46088", "");
        Test.ensureEqual(table.getFloatData(latIndex, row), 48.333f, "");
        Test.ensureEqual(table.getFloatData(lonIndex, row), -123.167f, "");
        ...
        Test.ensureEqual(table.getFloatData(atmpIndex, row), 8.0f, "");
        Test.ensureEqual(table.getFloatData(wtmpIndex, row), 7.6f, "");
        Test.ensureEqual(table.getFloatData(dewpIndex, row), 7.5f, "");
        Test.ensureEqual(table.getFloatData(visIndex, row), Float.NaN, ""); //(float)Math2.roundTo(18.5 * Math2.kmPerMile, decimalDigits[visIndex]), "");
    ...
    public static void test46088AddLastNDays(Table table) throws Exception {
        ...
        //#YY  MM DD hh mm WDIR WSPD GST  WVHT   DPD   APD MWD   PRES  ATMP  WTMP  DEWP  VIS PTDY  TIDE
        //#yr  mo dy hr mn degT m/s  m/s     m   sec   sec degT   hPa  degC  degC  degC   mi  hPa    ft
        //2013 02 26 15 20 140  7.0  8.0   0.6     4   3.4 113 1025.7   4.8   7.2   1.9   MM   MM    MM
        //2013 02 26 14 50 140  5.0  6.0   0.5     3   3.3 125 1025.8   5.0   7.2   1.9   MM +0.0    MM
        double seconds = Calendar2.isoStringToEpochSeconds("2013-02-26T15"); //rounded
        int row = table.getColumn(timeIndex).indexOf("" + seconds, 0);
        ...
        Test.ensureEqual(table.getFloatData(lonIndex, row), -123.167f, "");
        Test.ensureEqual(table.getDoubleData(depthIndex, row), 0, "");
        Test.ensureEqual(table.getFloatData(wdIndex, row), 140, "");
        Test.ensureEqual(table.getFloatData(wspdIndex, row), 7f, "");
        Test.ensureEqual(table.getFloatData(gstIndex, row), 8f, "");
        ...
        Test.ensureEqual(table.getFloatData(mwdIndex, row), 113f, "");
        Test.ensureEqual(table.getFloatData(aprsIndex, row), 1025.7f, "");
        Test.ensureEqual(table.getFloatData(atmpIndex, row), 4.8f, "");
        Test.ensureEqual(table.getFloatData(wtmpIndex, row), 7.2f, "");
        Test.ensureEqual(table.getFloatData(dewpIndex, row), 1.9f, "");
        Test.ensureEqual(table.getFloatData(visIndex, row), Float.NaN, ""); //(float)Math2.roundTo(18.5 * Math2.kmPerMile, decimalDigits[visIndex]), "");
        Test.ensureEqual(table.getFloatData(ptdyIndex, row), 0f, "");
        Test.ensureEqual(table.getFloatData(tideIndex, row), Float.NaN, "");//(float)Math2.roundTo(3.0 * Math2.meterPerFoot, decimalDigits[tideIndex]), "");
    ...
    public static void main(String args[]) throws Exception {
        ...
        //  Use DOS window "dir" or Linux ls instead of the GUI.
        //downloadNewHistoricalTxtFiles(ndbcHistoricalTxtDir);    //time varies, last done 2013-02-25

        // 3) *** get latest 45 day files
        //DON'T download45DayTextFiles after 45 days after last historicalTxt date.
        //download45DayTxtFiles(ndbc45DayTxtDir);  //15-30 minutes, last done 2013-02-25
        ...
        //    ndbcNcDir, ignoreStationsBefore, testMode); //M4700 ~1 hr, was ~3 hrs  //last done 2013-02-25
