...
public class NdbcMetStation  {
     ...
     */
    public static String firstNearRealTimeData = "2016-03-01T00:00:00";
...
<img src="/icons/text.gif" alt="[TXT]" /> <a href="42OTP.txt">42OTP.txt</a>               28-Aug-2005 14:23   24K  Standard Meteorological Data
    ...
    public static void test46088(Table table) throws Exception {
        ...
        //2016 03 01 01 20 200  1.0  2.0   0.5     3   3.3 127 1019.1   8.5   8.9   6.4   MM   MM    MM
        //2016 03 01 00 50 180  4.0  6.0   0.6     3   3.3  91 1019.2   9.4   9.0   5.9   MM -0.8    MM
        seconds = Calendar2.isoStringToEpochSeconds("2016-03-01T01"); //50 min rounds to next hour; usually test 01T01
        row = table.getColumn(timeIndex).indexOf("" + seconds, 0);
        Test.ensureEqual(table.getStringData(idIndex, row), "46088", "");
        Test.ensureEqual(table.getFloatData(latIndex, row), 48.333f, "");
        Test.ensureEqual(table.getFloatData(lonIndex, row), -123.167f, "");
        Test.ensureEqual(table.getDoubleData(depthIndex, row), 0, "");
        Test.ensureEqual(table.getDoubleData(wdIndex, row), 200, "");
        Test.ensureEqual(table.getFloatData(wspdIndex, row), 1f, "");
        ...
        Test.ensureEqual(table.getFloatData(mwdIndex, row), 127, "");  
        Test.ensureEqual(table.getFloatData(aprsIndex, row), 1019.1f, "");
        Test.ensureEqual(table.getFloatData(atmpIndex, row), 8.5f, "");
        Test.ensureEqual(table.getFloatData(wtmpIndex, row), 8.9f, "");
        Test.ensureEqual(table.getFloatData(dewpIndex, row), 6.4f, "");
        Test.ensureEqual(table.getFloatData(visIndex, row), Float.NaN, ""); //(float)Math2.roundTo(18.5 * Math2.kmPerMile, decimalDigits[visIndex]), "");
    ...
    public static void test46088AddLastNDays(Table table) throws Exception {
        ...
        //#YY  MM DD hh mm WDIR WSPD GST  WVHT   DPD   APD MWD   PRES  ATMP  WTMP  DEWP  VIS PTDY  TIDE
        //#yr  mo dy hr mn degT m/s  m/s     m   sec   sec degT   hPa  degC  degC  degC   mi  hPa    ft
        //2016 03 25 21 20 130  1.0  1.0   0.4     8   5.4 269 1023.2   9.8  10.9   3.9   MM   MM    MM
        //2016 03 25 20 50  MM  0.0  1.0   0.5     9   5.4 255 1023.5   9.9  10.4   4.4   MM +0.0    MM
        double seconds = Calendar2.isoStringToEpochSeconds("2016-03-25T21"); //rounded
        int row = table.getColumn(timeIndex).indexOf("" + seconds, 0);
        ...
        Test.ensureEqual(table.getFloatData(apdIndex, row), 5.4f, "");
        Test.ensureEqual(table.getFloatData(mwdIndex, row), 269, "");
        Test.ensureEqual(table.getFloatData(aprsIndex, row), 1023.2f, "");
        Test.ensureEqual(table.getFloatData(atmpIndex, row), 9.8f, "");
        Test.ensureEqual(table.getFloatData(wtmpIndex, row), 10.9f, "");
        Test.ensureEqual(table.getFloatData(dewpIndex, row), 3.9f, "");
        Test.ensureEqual(table.getFloatData(visIndex, row), Float.NaN, ""); //(float)Math2.roundTo(18.5 * Math2.kmPerMile, decimalDigits[visIndex]), "");
    ...
    public static void main(String args[]) throws Exception {
        ...
        //  Use DOS window "dir" or Linux ls instead of the GUI.
        //downloadNewHistoricalTxtFiles(ndbcHistoricalTxtDir); //time varies, last done 2016-03-25

        // 3) *** get latest 45 day files
        //DON'T download45DayTextFiles after 45 days after last historicalTxt date.
        //download45DayTxtFiles(ndbc45DayTxtDir);  //15-30 minutes, last done 2016-03-25
        ...
        //    ndbcNcDir, ignoreStationsBefore, testMode); //M4700 ~1 hr, was ~3 hrs  //last done 2016-03-25
