 ...
 * Files. These files have multiple row_size variables, each with a sample_dimension attribute. The
 * files are non-CF-standard files because the multiple sample (obs) dimensions are to be decoded
 * and related to each other with this additional rule and promise that is not part of the CF DSG
 * specification: "you can associate a given e.g., temperature value (temp_obs dimension) with a
 * given depth value (z_obs dimension, the dimension with the most values), because: the temperature
 * row_size (for a given cast) will be either 0 or equal to the corresponding depth row_size (for
 * that cast) (that's the rule). So, if the temperature row_size isn't 0, then the n temperature
 * values for that cast relate directly to the n depth values for that cast (that's the promise)."
 *
 * <p>Also, the Principal_Investigator row_size variable doesn't have a sample_dimension attribute
 * and doesn't follow the above rule.
 *
...
public class EDDTableFromInvalidCRAFiles extends EDDTableFromFiles {
  ...
  /**
   * This generates a ready-to-use datasets.xml entry for an EDDTableFromInvalidCRAFiles. The XML
  ...
  public static String generateDatasetsXml(
    ...
    tStandardizeWhat =
        tStandardizeWhat < 0 || tStandardizeWhat == Integer.MAX_VALUE
            ...
            : tStandardizeWhat;
    dataSourceTable.readInvalidCRA(sampleFileName, null, tStandardizeWhat, null, null, null);
    ...
    if (ft != null
      ...
      if (col < 0) col = dataAddTable.findColumnNumber("Pressure");
      if (col < 0) col = dataAddTable.findColumnNumber("pressure");
      if (col >= 0)
