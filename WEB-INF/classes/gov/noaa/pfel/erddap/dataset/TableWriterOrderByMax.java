...
/**
 * TableWriterOrderByMax provides a way to sort the response table's rows, and just keep the row
 * where the value of the last sort variable is highest. For example, you could use
 * orderBy(\"stationID,time\") to just get the rows of data with each station's maximum time value.
 *
