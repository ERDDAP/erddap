...
/**
 * TableWriterWav provides a way to write a table to a .wav file 
 * (see Table.writeWav())
...
public class TableWriterWav extends TableWriter {
     ...
     *     The ouputStream is not procured until finish() is called.
    ...
    /**
     * This adds the current contents of table (a chunk of data) to the DataOutputStream.
    ...
    public void writeSome(Table table) throws Throwable {
        ...
        if (isLong) {
            ...
            for (int col = 0; col < nColumns; col++) 
                pas[col] = (LongArray)(table.getColumn(col));
            for (int row = 0; row < nRows; row++) 
                for (int col = 0; col < nColumns; col++) 
                    dos.writeInt((int)(pas[col].array[row] >> 32));
        } else {
    ...
    public void finish() throws Throwable {
        ...
        //write the wav file
        Table.writeWaveFile(fullDosName, nColumns, totalNRows,
            isLong? "int" : tClass, 
            globalAttributes, randomInt, 
            fullOutName, System.currentTimeMillis());
        ...
        //diagnostic
        if (verbose)
            String2.log("TableWriterWav done. TIME=" + 
