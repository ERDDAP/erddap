  ...
  void drawBar(Graphics g,
    ...
    if(orient_ == HORIZONTAL) {
      ...
      for(int i=yend; i <= yloc; i++) {
        System.out.println("ColorKey.drawBar i="+i+" P=" + (layer_.getXDtoP(i)) + 
            " trans=" + (sTrans.getTransU(layer_.getXDtoP(i))) +
            " color=" + Integer.toHexString(cm_.getColor(sTrans.getTransU(layer_.getXDtoP(i))).getRGB()));
        g.setColor(cm_.getColor(sTrans.getTransU(layer_.getXDtoP(i))));
