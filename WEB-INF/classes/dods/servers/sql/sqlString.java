 ...
 * @see BaseType
 */
public class sqlString extends SDString {
    ...
    public boolean read(String datasetName, Object specialO)
        ...
        try {
            ...
            switch (rsmd.getColumnType(column)) {
                ...
                case Types.CHAR:
                case Types.VARCHAR:
                    b = result.getString(column);
                    break;
                case Types.DATE:
                    Date d = result.getDate(column);
                    if(d!=null)
                        b = d.toString();
                    break;
                case Types.TIME:
                    Time t = result.getTime(column);
                    if(t!=null)
                        b = t.toString();
                    break;
                ...
                case Types.LONGVARCHAR:
                    ...
                    if (Debug.isSet("sqlRead"))
                        System.out.println("\n\nWARNING! Attempting to read a type " +
                                rsmd.getColumnTypeName(column) +
                                " (JDBC type: LONGVARCHAR) into a String variable. " +
                                "This is type may be better represented as an Array, as " +
                                "it could be quite large (multiple Gigabytes).");
                    b = result.getString(column);
        ...
        if (Debug.isSet("sqlRead")) System.out.println("String received from SQl Query: \"" + b + "\"");
