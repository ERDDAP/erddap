...
/**
 * Holds a DODS Server <code>sqlF64</code> value.
 *
 * @version $Revision: 1.3.2.1 $
 * @author ndp
 * @see BaseType
 */
public class sqlF64 extends SDFloat64 {

    //private static final boolean _Debug = false;

    /** Constructs a new <code>sqlF64</code>. */
    public sqlF64() {
        super();
    }

    /**
     * Constructs a new <code>sqlF64</code> with name <code>n</code>.
     * @param n the name of the variable.
     */
    public sqlF64(String n) {
        super(n);
    }
    ...
    public boolean read(String datasetName, Object specialO)
        ...
        // Is the one to read
        sqlResponse sqlRes = (sqlResponse) specialO;
        ResultSet result = sqlRes.getResultSet();
        double b;
        int column = sqlRes.getCurrentColumn();
        ...
        try {
            ...
            switch (rsmd.getColumnType(column)) {
                ...
                case Types.FLOAT:
                case Types.REAL:
                    b = result.getFloat(column);
                    break;

                case Types.DOUBLE:

                    b = result.getDouble(column);
                    break;
                ...
                case Types.NUMERIC:
                    ...
                    BigDecimal bigOne;
                    bigOne = result.getBigDecimal(column);

                    if(bigOne != null){
                        //WARNING! LOSS OF PRECISION RIGHT HERE!
                        b = bigOne.doubleValue();
                    }
                    else {
                        b = Double.NaN;
                    }
                    if (Debug.isSet("sqlRead"))
                        System.out.println("WARNING: This conversion (from NUMERIC to Float64) may"+
        ...
        if (Debug.isSet("sqlRead")) System.out.println("Float64 received from SQl Query: \"" + b + "\"");
