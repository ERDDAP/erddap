...
public class BooleanPrimitiveVector extends PrimitiveVector implements Cloneable {
  ...
  /**
   * Returns a clone of this <code>BooleanPrimitiveVector</code>. A deep copy is performed on all
  ...
  /**
   * Writes data to a <code>DataOutputStream</code>. This method is used on the server side of the
   * DODS client/server connection, and possibly by GUI clients which need to download DODS data,
   * manipulate it, and then resave it as a binary file.
   *
   ...
   * @exception IOException thrown on any <code>OutputStream</code> exception.
   */
  public void externalize(DataOutputStream sink, int start, int stop, int stride)
      throws IOException {
    for (int i = start; i <= stop; i += stride) sink.writeBoolean(vals[i]);
  }
