 ...
 * DSequence</code>, <code>DGrid</code>, and <code>DList</code>).
 *
...
public abstract class DConstructor extends BaseType {
  ...
  /**
   * Return an Enumeration that can be used to iterate over all of the members of the class. Each
   * implementation must define what this means. The intent of this method is to support operations
   * on all members of a Structure, Seqeunce or Grid that can be performed equally. So it is not
   * necessary that this methods be usable, for example, when the caller needs to know that it s
   * dealing with the Array part of a grid.
   *
