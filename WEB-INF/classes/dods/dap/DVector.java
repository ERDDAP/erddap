 ...
 * </code> and <code>DArray</code>. This class uses a <code>PrimitiveVector</code> to hold the data
 * and deserialize it, thus allowing more efficient storage to be used for the primitive types.
 *
...
public abstract class DVector extends BaseType implements ClientIO {
  /** The values in this <code>DVector</code>, stored in a <code>PrimitiveVector</code>. */
  ...
  /**
   * Returns a clone of this <code>DVector</code>. A deep copy is performed on all variables inside
   * the <code>DVector</code>.
   *
  ...
  /**
   * Returns the <code>PrimitiveVector</code> for this vector. This can be cast to the appropriate
   * type and used by a DODS client to read or set individual values in the vector.
   *
  ...
  public synchronized void deserialize(DataInputStream source, ServerVersion sv, StatusUI statusUI)
    ...
    if (length < 0) throw new DataReadException("Negative array length read.");
