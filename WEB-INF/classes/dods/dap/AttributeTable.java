 ...
 * </code> object. For more information on the types of data which can be stored in an attribute,
 * including aliases and other <code>AttributeTable</code> objects, see the documentation for <code>
 * Attribute</code>.
 *
 * <p>The attribute tables have a standard printed representation. There is a <code>print</code>
 * method for writing this form and a <code>parse</code> method for reading the printed form.
 *
 * <p>An <code>AttributeTable</code>'s print representation might look like:
 *
 * <pre>
 ...
 * </pre>
 *
 * Here, <em>long_name</em> and <em>Investigators</em> are simple attributes, <em>actual_range</em>
 * is a container attribute, and <em>New_Attribute</em> is an alias pointing to
 * <em>Old_Attribute</em>.
 *
...
public class AttributeTable implements Cloneable {
  ...
  /**
   * Returns a clone of this <code>AttributeTable</code>. A deep copy is performed on all <code>
   * Attribute</code> and <code>AttributeTable</code> objects inside the <code>AttributeTable</code>
   * .
   *
  ...
  /**
   * Returns an <code>Enumeration</code> of the attribute names in this <code>AttributeTable</code>.
   * Use the <code>getAttribute</code> method to get the <code>Attribute</code> for a given name.
   *
   ...
   * @return the <code>Attribute</code> with the specified name, or null if there is no matching
   *     <code>Attribute</code>.
   * @see Attribute
   */
  ...
  /**
   * Adds an attribute to the table. If the given name already refers to an attribute, and the
   * attribute has a vector value, the given value is appended to the attribute vector. Calling this
   * function repeatedly is the way to create an attribute vector.
   *
   * <p>The function throws an exception if the attribute is a container, or if the type of the
   * input value does not match the existing attribute's type and the <code>check</code> parameter
   ...
   * @exception AttributeExistsException thrown if an Attribute with the same name, but a different
   *     type was previously defined.
  ...
  public final void appendAttribute(String name, int type, String value, boolean check)
    ...
    if (a != null && (type != a.getType())) {
      ...
      throw new AttributeExistsException(
          "`" + name + "' previously defined with a different type.");
   ...
   *
   * <p>The function throws an exception if the attribute is a container, or if the type of the
   * input value does not match the existing attribute's type. Use the <code>appendContainer</code>
  ...
  /**
   * Create and append an attribute container to the table. A container is another <code>
   * AttributeTable</code> object.
   *
   * @param name the name of the container to add.
   * @return A pointer to the new <code>AttributeTable</code> object, or null if a container by that
   *     name already exists.
   */
  public final AttributeTable appendContainer(String name) {
    // return null if name already exists
    if (attr.get(name) != null) return null;
   ...
   * @param name The name of the already-existing attribute to which the alias will refer.
   * @exception NoSuchAttributeException thrown if the existing attribute could not be found.
   * @exception AttributeExistsException thrown if the new alias has the same name as an existing
   *     attribute.
   */
  ...
  /**
   * Print the attribute table on the given <code>PrintWriter</code> with four spaces of
   * indentation.
   *
   * @param os the <code>PrintWriter</code> to use for output.
   */
  ...
  /**
   * Print the attribute table on the given <code>OutputStream</code> with four spaces of
   * indentation.
   *
   * @param os the <code>OutputStream</code> to use for output.
   */
