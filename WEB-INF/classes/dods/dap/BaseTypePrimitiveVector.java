...
public class BaseTypePrimitiveVector extends PrimitiveVector implements Cloneable {
  ...
  /**
   * Returns a clone of this <code>BaseTypePrimitiveVector</code>. A deep copy is performed on all
  ...
  public void externalize(DataOutputStream sink, int start, int stop, int stride)
      throws IOException {
    for (int i = start; i <= stop; i += stride) ((ClientIO) vals[i]).externalize(sink);
  }
