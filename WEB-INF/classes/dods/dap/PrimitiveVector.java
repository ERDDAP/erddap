...
/**
 * A helper class for <code>DVector</code>. It allows <code>DVector</code>, and by extension, <code>
 * DArray</code> and <code>DList</code>, to use more efficient primitive types to hold large arrays
 * of data. A <code>PrimitiveVector</code> class is defined for each primitive type supported in
 * DODS, and a <code>BaseTypePrimitiveVector</code> class handles <code>DArray</code>s and <code>
 * DList</code>s of compound types.
 *
 ...
 * @see DVector
 */
public abstract class PrimitiveVector implements ClientIO, Cloneable {
  /**
   * Template variable to use for <code>printDecl</code> and <code>deserialize</code> (<code>
   * BaseTypePrimitiveVector</code> only).
   */
  ...
  /**
   * Returns a clone of this <code>PrimitiveVector</code>. A deep copy is performed on all data
   * inside the variable.
   *
  ...
  /**
   * Sets the number of elements in the array. Allocates a new primitive array of the desired size.
   * Note that if this is called multiple times, the old array and its contents will be lost.
   *
   ...
   * @param sv The <code>ServerVersion</code> returned by the server. (used by <code>DSequence
   *     </code> to determine which protocol version was used).
   * @param statusUI The <code>StatusUI</code> object to use for GUI updates and user cancellation
   *     notification (may be null).
   * @exception DataReadException when invalid data is read, or if the user cancels the download.
   ...
   * @exception IOException thrown on any <code>OutputStream</code> exception.
   */
  // abstract public void externalize(DataOutputStream sink, int start, int stop, int stride) throws
  // IOException;

  /**
   * Write the variable's declaration in a C-style syntax. This method is used to create textual
   * representation of the Data Descriptor Structure (DDS).
   *
  ...
  public final void printDecl(
      PrintWriter os, String space, boolean print_semi, boolean constrained) {
  ...
  /**
   * Prints the value of all variables in this vector. This method is primarily intended for
  ...
  /**
   * Prints the value of a single variable in this vector. method is used by <code>DArray</code>'s
   * <code>printVal</code> method.
   *
   * @param os the <code>PrintWriter</code> on which to print the value.
   ...
   * <h2><i>WARNING:</i></h2>
   *
   * Because this method breaks encapsulation rules the user must beware! If we (the DODS prgramming
   * team) choose to change the internal representation(s) of these types your code will probably
   * break.
   *
   * <p>This method is provided as an optimization to eliminate massive copying of data.
   */
