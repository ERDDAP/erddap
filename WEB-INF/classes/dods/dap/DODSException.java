 ...
 *
 * <p>Unlike the other DODS exceptions, this one contains extra methods to get the various fields
 * sent by the server, and a <code>parse</code> method to parse the <code>Error</code> sent from the
 * server.
 *
...
public class DODSException extends Exception {
  ...
  /**
   * Reads an Error description from the named InputStream. This method calls a generated parser to
   * interpret an ASCII representation of an <code>Error</code>, and regenerate it as a <code>
   * DODSException</code>.
   *
  ...
  /**
   * Print the Error message on the given <code>PrintWriter</code>. This code can be used by
   * servlets to throw DODSException to client.
   *
  ...
  public void print(PrintWriter os) {
    ...
    if (errorMessage.charAt(0) == '"') os.println("    message = " + errorMessage + ";");
    else os.println("    message = \"" + errorMessage + "\";");
    os.println("};");
  }
