...
/**
 * SortedTable looks exactly like Hashtable but preserves the insertion order of elements. While
 * this results in slower performance, it ensures that the DAS will always be printed in the same
...
public final class SortedTable extends Dictionary {
   ...
   * @return the value to which the key is mapped, or null if the key is not mapped to any value in
   *     the table.
   */
  @Override
   ...
   * @return the previous value to which the key is mapped, or null if the key did not have a
   *     previous mapping.
  ...
  /**
   * Removes the key (and its corresponding value) from this table. If the key is not in the table,
   * do nothing.
   *
  ...
  /**
   * Returns a Vector containing the elements in the SortedTable. This is used for more efficient
   * implementation of dods.dap.Util.uniqueNames() by dods.dap.DDS.checkSemantics()
   *
