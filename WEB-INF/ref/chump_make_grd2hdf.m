...
a=load(infile);
lon = [min(a(:,1)):delx:max(a(:,1))];
lat = [min(a(:,2)):dely:max(a(:,2))];

% reshape grid
var = reshape(squeeze(a(:,3)),length(lon),length(lat));
...
[satellite, sensor, origin] = lookup_data_source(data_code);
[data_description, data_units] = lookup_data_id(data_id);
...
hdfsd('setattr',sds_id,'fraction_digits',int32(2));
