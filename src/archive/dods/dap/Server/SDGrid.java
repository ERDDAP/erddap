...
public abstract class SDGrid extends DGrid implements ServerArrayMethods, RelOps {
  ...
  /**
   * When projected (using whatever the current constraint provides in the way of a projection) am I
   * still a Grid?
   *
  ...
  public boolean projectionYieldsGrid() {
    ...
    if (!((SDArray) arrayVar).isProject()) return false;
    ...
    while (valid && e.hasMoreElements() && aDims.hasMoreElements()) {
      ...
      if (thisDim.getSize() > 0) {
        ...
        if (mapArray.isProject()) { // This map vector better be projected!
          ...
          valid =
              thisMap.getStart() == thisDim.getStart()
        ...
        // System.out.println("Dimension empty. Verifing cooresponding Map vector not
        // projected...");
  ...
  public int projectedComponents(boolean constrained) {
    ...
    if (constrained) {
      ...
      while (e.hasMoreElements()) {
        if (((SDArray) e.nextElement()).isProject()) comp++;
      }
    } else {
      comp = 1 + mapVars.size();
    }
  ...
  public void printDecl(PrintWriter os, String space, boolean print_semi, boolean constrained) {
    ...
    if (constrained && projectedComponents(true) == 0) return;
  ...
  public void printVal(PrintWriter os, String space, boolean print_decl_p) {
    ...
    if (((SDArray) arrayVar).isProject()) arrayVar.printVal(os, "", false);
  ...
  /**
   * Adds a variable to the container. This overrides the same method in the parent class <code>
   * DGrid</code> in order to add array projection functionality.
   *
   ...
   *     </code> otherwise.
   * @param all If <code>true</code>, set the Project property of all the members (and their
   *     children, and so on).
   * @see CEEvaluator
   */
  @Override
   ...
   * current projection for this variable should be <code>false</code>.
   *
   * <p>This is equivalent to setProjection(<code>state</code>, <code>true</code>).
   *
   * @param state <code>true</code> if the variable is part of the current projection, <code>false
   *     </code> otherwise.
   * @see CEEvaluator
   ...
   * will not have sensible responses to all of the relational operators (e.g. DGrid won't know how
  ...
  public boolean not_equal(BaseType bt)
    ...
    throw new InvalidOperatorException(
        "Not Equals (!=) operator does not work with the type SDGrid!");
  }
  ...
  public boolean greater(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "Greater Than (>)operator does not work with the type SDGrid!");
  }
  ...
  public boolean greater_eql(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "GreaterThan or equals (<=) operator does not work with the type SDGrid!");
  }
  ...
  public boolean less_eql(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "LessThan oe equals (<=) operator does not work with the type SDGrid!");
  }
  ...
  public void serialize(String dataset, DataOutputStream sink, CEEvaluator ce, Object specialO)
    ...
    if (ce.evalClauses(specialO)) {
      ...
      for (Enumeration e = mapVars.elements(); e.hasMoreElements(); ) {
        ServerMethods sm = (ServerMethods) e.nextElement();
        if (sm.isProject()) sm.serialize(dataset, sink, ce, specialO);
      }
    }
  }
   ...
   * retrieved and then the <code>DArrayDimension</code> associated with the <code>dimension</code>
   * specified is retrieved and the <code>start</code> <code>stride</code> and <code>stop</code>
  ...
  public void setProjection(int dimension, int start, int stride, int stop)
    ...
    try {
      ...
      throw new InvalidParameterException(
          "SDGrid.setProjection(): Bad Value for dimension!: " + e.getMessage());
    }
  }

  /**
   * Gets the <strong>start</strong> value for the projection of the <code>dimension</code>
   * indicated. The parameter <code>dimension</code> is checked against the instance of the <code>
   * SDArray</code> for bounds violation.
   *
   * @param dimension The dimension from whose projection to retrieve the <code>start</code> value.
   */
  @Override
  public int getStart(int dimension) throws InvalidParameterException {
    try {
      ...
      throw new InvalidParameterException(
          "SDGrid.getStart(): Bad Value for dimension!: " + e.getMessage());
    }
  }

  /**
   * Gets the <strong>stride</strong> value for the projection of the <code>dimension</code>
   * indicated. The parameter <code>dimension</code> is checked against the instance of the <code>
   * SDArray</code> for bounds violation.
   *
   * @param dimension The dimension from whose projection to retrieve the <code>stride</code> value.
   */
  @Override
  public int getStride(int dimension) throws InvalidParameterException {
    try {
      ...
      throw new InvalidParameterException(
          "SDGrid.getStride(): Bad Value for dimension!: " + e.getMessage());
    }
  }

  /**
   * Gets the <strong>stop</strong> value for the projection of the <code>dimension</code>
   * indicated. The parameter <code>dimension</code> is checked against the instance of the <code>
   * SDArray</code> for bounds violation.
   *
   * @param dimension The dimension from whose projection to retrieve the <code>stop</code> value.
   */
  @Override
  public int getStop(int dimension) throws InvalidParameterException {
    try {
      ...
      throw new InvalidParameterException(
          "SDGrid.getStop(): Bad Value for dimension!: " + e.getMessage());
    }
  }
}
