...
public abstract class SDList extends DList implements ServerMethods, RelOps {
   ...
   * will not have sensible responses to all of the relational operators (e.g. DList won't know how
  ...
  public boolean not_equal(BaseType bt)
    ...
    throw new InvalidOperatorException(
        "Not Equals (!=) operator does not work with the type SDList!");
  }
  ...
  public boolean greater(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "Greater Than (>)operator does not work with the type SDList!");
  }
  ...
  public boolean greater_eql(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "GreaterThan or equals (<=) operator does not work with the type SDList!");
  }
  ...
  public boolean less_eql(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "LessThan oe equals (<=) operator does not work with the type SDList!");
  }
   ...
   * <h2>Caution:</h2>
   *
   * When reading Lists of sequences (children of DSequence) it is crucial that it be handled with
   * great care. Sequences have been implemented so that only one instance (or row if you will) is
   * retained in memory at a given time. In order to correctly read a List of Sequences the read()
   * method for the List must create an instance of the Sequence for each member of the List,
   * typically by repeatedly cloning the template variable in the PrimitiveVector. The important
   * next step is: DO NOT attempt to read any data into the sequences from within the read() method
   * of the List. The sequence's data will get read, and constraint expressions applied when the
   * serialze() method of the List calls the serialize() method of the Sequence, which in turn calls
   * the read method of the Sequence. Good Luck!
   *
   ...
   * <h2>Caution:</h2>
   *
   * When serializing lists of sequences (children of DSequence) it is crucial that it be handled
   * with great care. Sequences have been implemented so that only one instance (or row if you will)
   * is retained in memory at a given time. In order to correctly serialize a list of sequences the
   * read() method for the List must create an instance of the sequence for each member of the list,
   * typically by repeatedly cloning the template variable in the PrimitiveVector. The important
   * next step is to NOT attempt to read any data into the sequences from within the read() method
   * of the List. The sequence's data will get read, and constraint expressions applied when the
   * serialze() method of the List calls the serialize() method of the sequence. Good Luck!
   *
