...
public abstract class SDArray extends DArray implements ServerArrayMethods, RelOps {
   ...
   * <strong><i> All the various versions of printVal() will only print a value, or a value with
   * declaration, if the variable is in the projection.</i></strong> <br>
   * <br>
  ...
  /**
   * Given a size and a name, this function adds a dimension to the array. For example, if the
   * <code>SDArray</code> is already 10 elements long, calling <code>appendDim</code> with a size of
   * 5 will transform the array into a 10x5 matrix. Calling it again with a size of 2 will create a
   * 10x5x2 array, and so on. This overloads <code>appendDim</code> of <code>DArray</code> so that
   * projection information need for the server side implementation can be handled.
   *
  ...
  /**
   * Set the state of this variable's projection. <code>true</code> means that this variable is part
   * of the current projection as defined by the current constraint expression, otherwise the
   * current projection for this variable should be <code>false</code>.
   *
   * @param state <code>true</code> if the variable is part of the current projection, <code>false
   *     </code> otherwise.
  ...
  /**
   * Is the given variable marked as projected? If the variable is listed in the projection part of
   * a constraint expression, then the CE parser should mark it as <em>projected</em>. When this
   * method is called on such a variable it should return <code>true</code>, otherwise it should
   * return <code>false</code>.
   *
   * @return <code>true</code> if the variable is part of the current projections, <code>false
   *     </code> otherwise.
   * @see CEEvaluator
  ...
  /**
   * The RelOps interface defines how each type responds to relational operators. Most (all?) types
   * will not have sensible responses to all of the relational operators (e.g. DArray won't know how
   * to match a regular expression but DString will). For those operators that are nonsensical a
  ...
  public boolean not_equal(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "Not Equals (!=) operator does not work with the type SDArray!");
  }
  ...
  public boolean greater(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "Greater Than (>)operator does not work with the type SDArray!");
  }
  ...
  public boolean greater_eql(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "GreaterThan or equals (<=) operator does not work with the type SDArray!");
  }

  @Override
  public boolean less(BaseType bt) throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "LessThan (<) operator does not work with the type SDArray!");
  }
  ...
  public boolean less_eql(BaseType bt)
      throws InvalidOperatorException, RegExpException, SBHException {
    throw new InvalidOperatorException(
        "LessThan oe equals (<=) operator does not work with the type SDArray!");
  }
   ...
   *
   * @param state If <code>true</code> then the variable is considered a synthetic variable and no
   *     part of DODS will ever try to read it from a file, otherwise if <code>false</code> the
   *     variable is considered a normal variable whose value should be read using the <code>read()
   *     </code> method. By default this property is false.
   * @see #isSynthesized()
   ...
   *
   * @return <code>true</code> if this is a synthetic variable, <code>false</code> otherwise.
   */
  @Override
  ...
  /**
   * Set the Read property. A normal variable is read using the <code>read()</code> method. Once
   * read the <em>Read</em> property is <code>true</code>. Use this function to manually set the
   * property value. By default this property is false.
   *
   * @param state <code>true</code> if the variable has been read, <code>false</code> otherwise.
   * @see #isRead()
   * @see #read(String, Object)
   */
  @Override
   ...
   *
   * @return <code>true</code> if the variable has been read, <code>false</code> otherwise.
   * @see #read(String, Object)
   * @see #setRead(boolean)
   */
  @Override
   ...
   *
   * @param datasetName String identifying the file or other data store from which to read a vaue
   *     for this variable.
   * @param specialO This <code>Object</code> is a goody that is used by Server implementations to
   *     deliver important, and as yet unknown, stuff to the read method. If you don't need it, make
   *     it a <code>null</code>.
   * @return <code>true</code> if more data remains to be read, otherwise <code>false</code>. This
   *     is an abtsract method that must be implemented as part of the installation/localization of
   *     a DODS server.
  ...
  /**
   * Server-side serialization for DODS variables (sub-classes of <code>BaseType</code>). This does
   * not send the entire class as the Java <code>Serializable</code> interface does, rather it sends
   * only the binary data values. Other software is responsible for sending variable type
   * information (see <code>DDS</code>).
   *
   * <p>Writes data to a <code>DataOutputStream</code>. This method is used on the server side of
   * the DODS client/server connection, and possibly by GUI clients which need to download DODS
   * data, manipulate it, and then re-save it as a binary file.
   *
   * <h2>Caution:</h2>
   *
   * When serializing arrays of sequences (children of DSequence) it is crucial that it be handled
   * with great care. Sequences have been implemented so that only one instance (or row if you will)
   * is retained in memory at a given time. In order to correctly serialize an array of sequences
   * the read() method for the array must create an instance of the sequence for each member of the
   * array, typically by repeatedly cloning the template variable in the PrimitiveVector. The
   * important next step is to NOT attempt to read any data into the sequences from within the
   * read() method of the parent array. The sequence's data will get read, and constraint
   * expressions applied when the serialze() method of the array calls the serialize method of the
   * sequence. Good Luck!
   *
  ...
  public void serialize(String dataset, DataOutputStream sink, CEEvaluator ce, Object specialO)
    ...
    if (!isRead()) read(dataset, specialO);
  ...
  /**
   * Set the projection information for this dimension. The <code>DArrayDimension</code> associated
   * with the <code>dimension</code> specified is retrieved and the <code>start</code> <code>stride
   * </code> and <code>stop</code> parameters are passed to its <code>setProjection()</code> method.
   *
  ...
  /**
   * Gets the <strong>start</strong> value for the array projection. The parameter <code>dimension
   * </code> is checked against the instance of the <code>SDArray</code> for bounds violation.
   */
  @Override
  ...
  /**
   * Gets the <strong>stride</strong> value for the array projection. The parameter <code>dimension
   * </code> is checked against the instance of the <code>SDArray</code> for bounds violation.
   */
  @Override
  ...
  /**
   * Gets the <strong>stop</strong> value for the array projection. The parameter <code>dimension
   * </code> is checked against the instance of the <code>SDArray</code> for bounds violation.
   */
  @Override
