...
public class JpegEncoder extends Frame {
                ...
                for (comp = 0; comp < JpegObj.NumberOfComponents; comp++) {
                    ...
                    for (i = 0; i < JpegObj.VsampFactor[comp]; i++) {
                        for (j = 0; j < JpegObj.HsampFactor[comp]; j++) {
                            ...
                            //                           dctArray3 = zeroArray;
                            //                        }
                            Huf.HuffmanBlockEncoder(out, dctArray3,
     ...
     * @param out DOCUMENT ME!
     */
    public void WriteEOI(OutputStream out) throws IOException {
    ...
    public void WriteHeaders(OutputStream out) throws IOException {
        ...
        for (i = 0; i < SOF[9]; i++) {
            ...
            SOF[index++] =
                (byte) ((JpegObj.HsampFactor[i] << 4) + JpegObj.VsampFactor[i]);
        ...
        for (i = 0; i < SOS[4]; i++) {
            ...
            SOS[index++] =
                (byte) ((JpegObj.DCtableNumber[i] << 4)
     ...
     * @param out DOCUMENT ME!
     */
    void WriteMarker(byte[] data, OutputStream out) throws IOException {
        out.write(data, 0, 2);
    }
     ...
     * @param out DOCUMENT ME!
     */
    void WriteArray(byte[] data, OutputStream out) throws IOException {
        int length =
            (((int) (data[2] & 0xFF)) << 8) + (int) (data[3] & 0xFF) + 2;
        ...
        for (i = 0; i < 8; i++) {
            for (j = 0; j < 8; j++) {
                ...
                DivisorsLuminance[index] =
                    (double) ((double) 1.0 / ((double) quantum_luminance[index] * AANscaleFactor[i] * AANscaleFactor[j] * (double) 8.0));
                index++;
            }
        }
        ...
        for (i = 0; i < 8; i++) {
            for (j = 0; j < 8; j++) {
                ...
                DivisorsChrominance[index] =
                    (double) ((double) 1.0 / ((double) quantum_chrominance[index] * AANscaleFactor[i] * AANscaleFactor[j] * (double) 8.0));
                index++;
            }
        }
                ...
                for (x = 0; x < 8; x++) {
                    for (y = 0; y < 8; y++) {
                        output[v][u] += (((double) input[x][y]) * Math.cos(((double) ((2 * x)
                            + 1) * (double) u * Math.PI) / (double) 16) * Math
                        .cos(((double) ((2 * y) + 1) * (double) v * Math.PI) / (double) 16));
                    }
                }
...
class Huffman {
    ...
    * data.
    **/
    public void HuffmanBlockEncoder(OutputStream out, int[] zigzag, int prec,
        int DCcode, int ACcode) throws IOException {
        int temp;
        int temp2;
        int nbits;
        int k;
        ...
        bufferIt(out, ((int[][]) DC_matrix[DCcode])[nbits][0],
        ...
        for (k = 1; k < 64; k++) {
            if ((temp = zigzag[jpegNaturalOrder[k]]) == 0) {
                ...
                while (r > 15) {
                    bufferIt(out, ((int[][]) AC_matrix[ACcode])[0xF0][0],
                ...
                i = (r << 4) + nbits;
                bufferIt(out, ((int[][]) AC_matrix[ACcode])[i][0],
        ...
        if (r > 0) {
            bufferIt(out, ((int[][]) AC_matrix[ACcode])[0][0],
    ...
    void bufferIt(OutputStream out, int code, int size) throws IOException {
        int PutBuffer = code;
    ...
    */
    public void initHuf() {
        ...
        * [][0] is the tcode   [][1] is the number of bit
        */
        p = 0;
        ...
        /*
        * Init of the AC hufmann tcode for the chrominance
        * matrix [][][0] is the tcode & matrix[][][1] is the number of bit needed
        */
        p = 0;
        ...
        */
        ...
        /*
        * Init of the AC hufmann tcode for luminance
        * matrix [][][0] is the tcode & matrix[][][1] is the number of bit
        */
        p = 0;
    ...
    private void getYCCArray() {
        ...
        for (y = 0; y < imageHeight; ++y) {
            for (x = 0; x < imageWidth; ++x) {
                ...
                Y[y][x] =
                    (float) (((0.299 * (float) r) + (0.587 * (float) g)
                    + (0.114 * (float) b)));
                Cb1[y][x] =
                    128
        ...
        for (outrow = 0; outrow < compHeight[comp]; outrow++) {
            ...
            for (outcol = 0; outcol < compWidth[comp]; outcol++) {
                output[outrow][outcol] =
                    ...
                    + C[inrow][incol++] + C[inrow--][incol++] + (float) bias) / (float) 4.0;
                bias ^= 3;
            }
