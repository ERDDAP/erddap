...
// ==============================================================================
/**
 * This is the central class of a JDK 1.1 compatible GIF encoder that, AFAIK, supports more features
 * of the extended GIF spec than any other Java open source encoder. Some sections of the source are
 * lifted or adapted from Jef Poskanzer's <cite>Acme GifEncoder</cite> (so please see the <a
 * href="../readme.txt">readme</a> containing his notice), but much of it, including nearly all of
 * the present class, is original code. My main motivation for writing a new encoder was to support
 * animated GIFs, but the package also adds support for embedded textual comments.
 *
 * <p>There are still some limitations. For instance, animations are limited to a single global
 ...
 *
 * <p>The encoder, though very usable in its present form, is at bottom only a partial
 * implementation skewed toward my own particular needs. Hence a couple of caveats are in order. (1)
 * During development it was in the back of my mind that an encoder object should be reusable -
 * i.e., you should be able to make multiple calls to encode() on the same object, with or without
 ...
 *
 * <p>There follow a couple of simple examples illustrating the most common way to use the encoder,
 ...
 * features.
 *
 * <p><strong>Animated GIF Example</strong>
 *
 * <pre>
 ...
 *  </pre>
 *
 * <p>Bob Simons (was bob.simons@noaa.gov, now BobSimons2.00@gmail.com) added color palette options
 * 2005-04-11. You can get the original Gif89Encoder for free from http://jmge.net/java/gifenc/ .
 *
...
public class Gif89Encoder {
  ...
  public static final int DYNAMIC_PALETTE =
      GifColorTable.DYNAMIC_PALETTE; // use first 256 colors found in image
  public static final int WEB216_PALETTE =
      GifColorTable.WEB216_PALETTE; // find nearest colors in standard web 216 color palette
  ...
  // ----------------------------------------------------------------------------
  /** Use this constructor if you want to specify the WEB216_PALETTE. Added by Bob Simons. */
  public Gif89Encoder(int palette) {
    // empty color table puts us into "palette autodetect" mode
    colorTable = new GifColorTable(palette);
  }
  ...
  /**
   * This is the like the constructor above, but allows you to specify the palette. Added by Bob
   * Simons.
   */
  public Gif89Encoder(Image static_image, int palette) throws IOException {
    this(palette);
    addFrame(static_image);
  }
   ...
   * ARBG pixels.
   *
   * <p>Use of this constructor imposes a couple of restrictions: (1) Frame objects can't be of type
  ...
  // ----------------------------------------------------------------------------
  /**
   * Convenience constructor for encoding a static GIF from index-model data. Adds a single frame as
   * specified.
   *
   * @param colors Array of color values; no more than 256 colors will be read, since that's the
   *     limit for a GIF.
  ...
  // ----------------------------------------------------------------------------
  /**
   * Add a Gif89Frame frame to the end of the internal sequence. Note that there are restrictions on
   * the Gif89Frame type: if the encoder object was constructed with an explicit color table, an
   * attempt to add a DirectGif89Frame will throw an exception.
   *
   * @param gf An externally constructed Gif89Frame.
   * @exception IOException If Gif89Frame can't be accommodated. This could happen if either (1) the
   *     aggregate cross-frame RGB color count exceeds 256, or (2) the Gif89Frame subclass is
   ...
   *     color count exceeds 256, or (3) this encoder object was constructed with an explicit color
   *     table.
   */
   ...
   *     exceptions that can be thrown when adding an IndexGif89Frame <i>per se</i>. But I might add
   *     some pedantic check later, to justify the generality :)
   */
   ...
   *
   * @param count Number of times to play sequence. Special value of 0 specifies indefinite looping.
   *     (Default: 1)
   */
  ...
  // ----------------------------------------------------------------------------
  /**
   * A convenience method for setting the "animation speed". It simply sets the delay parameter for
   * each frame in the sequence to the supplied value. Since this is actually frame-level rather
   * than animation-level data, take care to add your frames before calling this method.
   *
   ...
   * to the passed stream. Multiple calls are permissible if for some reason that is useful to your
   * application. (The method simply encodes the current state of the object with no thought to
   * previous calls.)
   *
  ...
  public void encode(OutputStream out) throws IOException {
    ...
    for (int i = 0; i < nframes; ++i)
      ((Gif89Frame) vFrames.elementAt(i))
          .encode(out, is_sequence, colorTable.getDepth(), colorTable.getTransparent());
  ...
  // ----------------------------------------------------------------------------
  /**
   * A simple driver to test the installation and to demo usage. Put the JAR on your classpath and
   * run ala
   *
   * <blockquote>
   *
   ...
   * </blockquote>
   *
   * The filename must be either (1) a JPEG file with extension 'jpg', for conversion to a static
   * GIF, or (2) an ISO-8859-1 file containing a list of GIFs and/or JPEGs, one per line, to be
   * combined into an animated GIF. The output will appear in the current directory as
   * 'gif89out.gif'.
   *
   * <p>(N.B. This test program will abort if the input file(s) exceed(s) 256 total RGB colors, so
   * in its present form it has no value as a generic JPEG to GIF converter. Also, when multiple
   * files are input, you need to be wary of the total color count, regardless of file type.)
   *
   * @param args Command-line arguments, only the first of which is used, as mentioned above.
   */
...
class GifColorTable {
  ...
  public static final int DYNAMIC_PALETTE = 0; // use first 256 colors found in image
  public static final int WEB216_PALETTE =
      1; // find nearest colors in standard web 216 color palette
  public static final int DITHERED_WEB216_PALETTE =
      2; // find nearest colors in standard web 216 color palette, with dithering
  private int palette = DYNAMIC_PALETTE;
  ...
  /** New constructor for use with WEB216_PALETTE. Added by Bob Simons. */
  GifColorTable(int palette) {
    if (palette != WEB216_PALETTE && palette != DITHERED_WEB216_PALETTE) palette = DYNAMIC_PALETTE;
    this.palette = palette;

    // then same as default constructor
    ciLookup = new ReverseColorMap(); // puts us into "auto-detect mode"
  }
  ...
  /**
   * Convert any rgb to a color in the standard 216 color web palette. Added by Bob Simons.
   *
  ...
  // blob
  // {0,  6, 39},
  // {12,18, 27},  //27 for 24+30; 39 for 36+42
  // {39,27, 48}};

  // {10, 25},
  // {25, 40}};
  /**
   * Convert any rgb to a color in the standard 216 color web palette (with dithering).
   *
   * @param rgb (the opacity value is ignored)
   ...
   * @return a color in the standard 216 color web palette (6 levels each of r, g, b).
   */
  public static int rgbToDitheredWeb216Color(int rgb, int x, int y) {
    int bias = dither[x % 4][y % 4];
    return (((rgb & 0xFF0000) + (bias << 16)) / 0x330000 * 0x330000)
        | (((rgb & 0xFF00) + (bias << 8)) / 0x3300 * 0x3300)
        | (((rgb & 0xFF) + bias) / 0x33 * 0x33);
  }

  // ----------------------------------------------------------------------------
  void processPixels(Gif89Frame gf) throws IOException {
    if (gf instanceof DirectGif89Frame) filterPixels((DirectGif89Frame) gf);
  ...
  private void filterPixels(DirectGif89Frame dgf) throws IOException {
    ...
    for (int i = 0; i < npixels; ++i) {
      ...
      if (palette == WEB216_PALETTE) {
        argb = 0xFF000000 | rgbToWeb216Color(argb);
      } else if (palette == DITHERED_WEB216_PALETTE) {
        argb = 0xFF000000 | rgbToDitheredWeb216Color(argb, i % width, i / width);
      }
