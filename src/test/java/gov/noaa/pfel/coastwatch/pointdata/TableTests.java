...
class TableTests {
   ...
   * Very large test file has discharge[time=758001][station=18]
   * 4vars[station=18],
  ...
  void testReadNcCFMATimeSeriesReversed(boolean readAsNcCF) throws Exception {
    ...
    String fileName = TableTests.class.getResource("/largeFiles/nccf/MATimeSeriesReversedDim.nc").getPath();
  ...
  void testJsonlCSV() throws Exception {
    ...
    results = table.dataToString();
    String expected = "aString,aChar,aByte,aUByte,aShort,aUShort,anInt,aUInt,aLong,aULong,aFloat,aDouble\n" +
    ...
    Test.ensureEqual(results, expected, "results=\n" + results);
    PAType tTypes[] = {
        PAType.STRING, PAType.STRING, PAType.BYTE, PAType.SHORT, PAType.SHORT, // char->String, unsigned ->
                                                                               // larger
                                                                               // signed
  ...
  void testASCII() throws Exception {
    ...
    for (int col = 2; col < n; col++) // skip first 2 columns which are intentionally initially stored in bigger
                                      // type
  ...
  void testHtml() throws Exception {
    ...
    // ** finally
    Math2.gc("Table (between tests)", 10000); // in a test. Do something useful while browser gets going to display
                                              // the
                                              // file.
    File2.delete(fileName);
  }
  ...
  void testFlatNc() throws Exception {
    ...
    // don't unpack
    table.readFlatNc(TEMP_DIR.toAbsolutePath().toString() + "/41015.nc", new String[] { "time", "BAR" }, 0); // standardizeWhat=0
    ...
    table.readFlatNc(TEMP_DIR.toAbsolutePath().toString() + "/41015.nc", new String[] { "time", "BAR" }, 1); // standardizeWhat=1
    table.subset(testColumns, testMin, testMax);
    Test.ensureEqual(table.nColumns(), 2, "");
    Test.ensureEqual(table.nRows(), 1, "");
    Test.ensureEqual(table.getColumnName(0), "time", "");
   ...
   * I note that loaddods documentation is at
   * https://www.opendap.org/user/mgui-html/mgui_36.html
  ...
  void testOpendapSequence() throws Exception {
    ...
    expected = "https://data.pmel.noaa.gov/pmel/erddap/tabledap/ChukchiSea_454a_037a_fcf4.das\" ;\n" +
    ...
    expected = "\t\t:time_coverage_end = \"2002-05-30T15:22:00Z\" ;\n" +
    ...
    expected = "cruise_id,longitude,latitude,time,cast_no,station_id,abund_m3,comments,counter_id,d_n_flag,gear_area,gear_mesh,gear_type,genus_species,life_stage,local_code,max_sample_depth,min_sample_depth,nodc_code,perc_counted,program,sample_id,vol_filt,water_depth\n"
        +
    ...
    results = table.dataToString();
    expected = "abund_m3,latitude,longitude\n" +
     ...
     * ???Is this a real NaN row, or a mistake in my code (e.g., end of sequence
     * beginning of next).
     ...
     * to find time,lat,lon,depth where there are valid values of
     * English_sole_LarvaeCount.
     * And requesting all data rows (so I can then filtering on my end) takes ~40
     * seconds
     * for 98 rows of data.
     * [Wasteful, but I believe Roy did it this way to conform to Dapper Conventions
     * so
     * data easily served by Dapper/DChart, see
     * http://oceanview.pfeg.noaa.gov/dchart (was http://las.pfeg...).]
     * 
     * 4) There are no 0 values for English_sole_LarvaeCount.
     * So how can one tell if people looked for English_sole_Larvae but didn't find
     * any?
     ...
     * [Because all variables are floats for simplicity (maybe for matlab or
     * fortran).]
     * 
     */
  }
  ...
  void testReadASCIISpeed() throws Exception {
    ...
    String fileName = TableTests.class.getResource("/data/points/ndbcMet2HistoricalTxt/41009h1990.txt").getPath();
    long time = 0;
    ...
    for (int attempt = 0; attempt < 4; attempt++) {
      ...
      String expected = "YY,MM,DD,hh,WD,WSPD,GST,WVHT,DPD,APD,MWD,BAR,ATMP,WTMP,DEWP,VIS\n" +
    ...
    if (time > 130)
      throw new SimpleException(
          "readASCII took too long (time=" + time + "ms > 130ms) (but often does when computer is busy).");
  }
  ...
  void testReadOpendapSequenceSpeed() throws Exception {
    ...
    String url = "https://coastwatch.pfeg.noaa.gov/erddap/tabledap/cwwcNDBCMet?" +
  ...
  void testSaveAsSpeed() throws Exception {
    ...
    String sourceName = TableTests.class.getResource("/data/points/ndbcMet2HistoricalTxt/41009h1990.txt").getPath();
  ...
  void testOrderByMinMax() throws Exception {
    ...
    for (int proc = 0; proc < 3; proc++) {
      ...
      if (proc == 0)
        table.orderByMax(vars);
      if (proc == 1)
        table.orderByMin(vars);
      if (proc == 2)
        table.orderByMinMax(vars);
      String results = table.dataToString();
  ...
  void testGetDapQueryParts() throws Exception {
    ...
    Test.ensureEqual(Table.getDapQueryParts("&&"), new String[] { "", "", "" }, "");
    Test.ensureEqual(Table.getDapQueryParts("a&b=R%26D"), new String[] { "a", "b=R&D" }, ""); // & visible
    Test.ensureEqual(Table.getDapQueryParts("a%26b=\"R%26D\""), new String[] { "a", "b=\"R&D\"" }, ""); // & encoded
    Test.ensureEqual(Table.getDapQueryParts("a%26b=\"R%26D\"%26c"), new String[] { "a", "b=\"R&D\"", "c" }, ""); // &
                                                                                                                 // encoded
  ...
  void testSubsetViaDapQuery() throws Exception {
    ...
    results = table.dataToString();
    expected = "Time,Longitude,Latitude,Double Data,Int Data,Short Data,Byte Data,Char Data,String Data\n" +
  ...
  void testAddIndexColumns() throws Exception {
    ...
    String results = table.dataToString();
    String expected = "_index_0,_index_1,_index_2\n" +
        "0,0,0\n" +
        "0,0,1\n" +
        "0,0,2\n" +
        "0,0,3\n" +
  ...
  void testReadMultidimNc() throws Exception {
    ...
    results = table.dataToString(5);
    expectedEnd = "DATA_TYPE,FORMAT_VERSION,HANDBOOK_VERSION,REFERENCE_DATE_TIME,DATE_CREATION,DATE_UPDATE,PLATFORM_NUMBER,PROJECT_NAME,PI_NAME,CYCLE_NUMBER,DIRECTION,DATA_CENTRE,DC_REFERENCE,DATA_STATE_INDICATOR,DATA_MODE,PLATFORM_TYPE,FLOAT_SERIAL_NO,FIRMWARE_VERSION,WMO_INST_TYPE,JULD,JULD_QC,JULD_LOCATION,LATITUDE,LONGITUDE,POSITION_QC,POSITIONING_SYSTEM,PROFILE_PRES_QC,PROFILE_TEMP_QC,PROFILE_PSAL_QC,VERTICAL_SAMPLING_SCHEME,CONFIG_MISSION_NUMBER,PRES,PRES_QC,PRES_ADJUSTED,PRES_ADJUSTED_QC,PRES_ADJUSTED_ERROR,TEMP,TEMP_QC,TEMP_ADJUSTED,TEMP_ADJUSTED_QC,TEMP_ADJUSTED_ERROR,PSAL,PSAL_QC,PSAL_ADJUSTED,PSAL_ADJUSTED_QC,PSAL_ADJUSTED_ERROR\n"
        +
    ...
    results = table.dataToString(3);
    expectedStart = "DATA_TYPE,FORMAT_VERSION,HANDBOOK_VERSION,REFERENCE_DATE_TIME,DATE_CREATION," +
    ...
    table.readMultidimNc(fiName, StringArray.fromCSV("LONGITUDE,PRES,PSAL_ADJUSTED_ERROR"),
        ...
        null, null, null); // conVars, conOps, conVals
    results = table.dataToString(3);
    expectedStart = "LONGITUDE,PRES,PSAL_ADJUSTED_ERROR\n" +
    ...
    table.removeRows(0, table.nRows() - 3);
    results = table.dataToString(5);
    expectedEnd = "LONGITUDE,PRES,PSAL_ADJUSTED_ERROR\n" +
    ...
    table.readMultidimNc(fiName,
        ...
        null, null, null); // conVars, conOps, conVals
    results = table.dataToString(3);
    expectedStart = "DATA_TYPE,FORMAT_VERSION,HANDBOOK_VERSION,REFERENCE_DATE_TIME,DATE_CREATION,DATE_UPDATE\n" +
  ...
  void testReadAsciiCsvFile() throws Exception {
    ...
    results = table.dataToString();
    expected = "aString,aChar,aBoolean,aByte,aShort,anInt,aLong,aFloat,aDouble\n" +
  ...
  void testReadAsciiSsvFile() throws Exception {
    ...
    table.readASCII(fileName, File2.ISO_8859_1,
        ...
        new String[] { "aDouble", "aString", "aByte" }, true); // load cols
    results = table.dataToString();
    expected = "aDouble,aString,aByte\n" +
        ",needs1space,\n" +
        "4.0E200,j,14\n" +
        "5.0E200,k,15\n" +
        "6.0E200,l,16\n";
  ...
  void testReadColumnarASCIIFile() throws Exception {
    ...
    String results, expected;
    StringArray sa = new StringArray();
    String fullFileName = TableTests.class.getResource("/data/columnarAsciiWithComments.txt").getPath();
    String skipHeaderToRegex = "END OF HEADER.*";
    String skipLinesRegex = "%.*";
    String colNames[] = {
    ...
    results = table.dataToString();
    expected = "aDouble,aString,aChar,aBoolean,aByte,aShort,anInt,aLong,aFloat\n" +
  ...
  void testNccsv() throws Exception {
    ...
    String expected1 = "*GLOBAL*,Conventions,\"COARDS, CF-1.6, ACDD-1.3, NCCSV-1.2\"\n" +
    ...
    String expected2 = "*GLOBAL*,Conventions,\"COARDS, CF-1.6, ACDD-1.3, NCCSV-1.2\"\n" +
        "*GLOBAL*,cdm_trajectory_variables,ship\n" +
        "*GLOBAL*,creator_email,bob.simons@noaa.gov\n" +
        "*GLOBAL*,creator_name,Bob Simons\n" +
        "*GLOBAL*,creator_type,person\n" +
    ...
    String2.log(">> sample_1.2.csv testChars isCharArray?" + (pa instanceof CharArray) + " isStringArray?"
        + (pa instanceof StringArray));
    results = table.saveAsNccsv(false, true, 0, Integer.MAX_VALUE); // don't catch scalars
    ...
    String expected3 = "*GLOBAL*,Conventions,\"COARDS, CF-1.6, ACDD-1.3, NCCSV-1.2\"\n" +
        "*GLOBAL*,cdm_trajectory_variables,ship\n" +
        "*GLOBAL*,creator_email,bob.simons@noaa.gov\n" +
        "*GLOBAL*,creator_name,Bob Simons\n" +
        "*GLOBAL*,creator_type,person\n" +
  ...
  void testReadAwsXmlFile() throws Exception {
    ...
    Table table = new Table();
    table.readAwsXmlFile(TableTests.class.getResource("/data/aws/xml/SNFLS-2012-11-03T20_30_01Z.xml").getPath());
    String results = table.toString();
    String expected = "{\n" +
        "dimensions:\n" +
        "\trow = 1 ;\n" +
  ...
  void testReadNDNc() throws Exception {
    ...
    String fiName = TableTests.class
    ...
    table.readNDNc(fiName, new String[] { "temperature", "lat", "salinity", "junk" }, 0, // standardizeWhat=0
        "depth", 100, 200);
    results = table.dataToString();
    expected = "time,depth,lat,lon,temperature,salinity\n" +
    ...
    expected = "row,unique_tag_id,PI,longitude,latitude,time,bottom_depth,common_name,date_public,line,position_on_subarray,project,riser_height,role,scientific_name,serial_number,stock,surgery_time,surgery_location,tagger\n"
        +
    ...
    fiName = TableTests.class.getResource("/data/points/ndbcMet2/historical/NDBC_51001_met.nc").getPath(); // implied
                                                                                                           // c:
  ...
  void testReadNDNc2() throws Exception {
    ...
    table.readNDNc(fiName, new String[] {
        ...
        "z", 100, 200);
    results = table.dataToString();
    expected = "z,Temperature,WOD_cruise_identifier,lat,lon,time\n" +
    ...
    table.readNDNc(fiName, new String[] {
        ...
        "z", 100, 200);
    results = table.dataToString();
    expected = "WOD_cruise_identifier,lat,lon,Project\n" +
    ...
    table.readNDNc(fiName, new String[] {
        ...
        "z", 100, 200);
    results = table.dataToString();
    expected = "z,WOD_cruise_identifier,lat,lon,Project\n" +
  ...
  void testReadVlenNc() throws Exception {
    ...
    Table table = new Table();
    String fiName = TableTests.class.getResource("/largeFiles/nccf/vlen/rr2_vlen_test.nc").getPath();
    // String2.log(NcHelper.ncdump(fiName, "-h"));
    String results, expectedStart, expectedEnd;
    /* */
  ...
  void testReadInvalidCRA() throws Exception {
    ...
    results = table.dataToString(5);
    expected = "z,Temperature,Temperature_WODflag\n" +
        "570.5,5.8,0\n" +
        "570.5,5.8,0\n" +
        "570.5,5.8,0\n" +
        "570.5,5.8,0\n" +
  ...
  /**
   * This tests reading an ncCF Contiguous Ragged Array file with 7(!)
   * sample_dimension's.
   */
  @org.junit.jupiter.api.Test
  ...
  void testReadNcCFPoint() throws Exception {
    ...
    Table table = new Table();
    String results, expected;
    String fileName = TableTests.class.getResource("/data/CFPointConventions/point/point-H.1/point-H.1.nc")
        .getPath();
    Attributes gatts;
    ...
    results = table.dataToString(5);
    expected = "obs,lat,lon,alt,time,temperature,humidity\n" +
    ...
    expected = "    coordinates=time lat lon alt\n" +
        "    long_name=Humidity\n" +
  ...
  void testReadNcCF1() throws Exception {
    ...
    // String2.log(table.toCSVString());
    results = table.dataToString(5);
    expected = "id,longitude,latitude,time,altitude,chlorophyll,chlorophyll_qc,oxygen,oxygen_qc,pressure,pressure_qc,salinity,salinity_qc,temperature,temperature_qc\n"
        +
    ...
    expected = "    actual_range=465958i,848984i\n" +
        "    cf_role=profile_id\n" +
    ...
    results = table.dataToString();
    expected = "longitude,latitude,time,altitude,temperature,temperature_qc,id\n" +
    ...
    for (int type = 0; type < 2; type++) {
      ...
      String fileName = TableTests.class.getResource("/data/nccf/" + (type == 0 ? "ncCF1b.nc" : "ncCFMA1b.nc"))
          .getPath();
      ...
      results = table.columnAttributes(0).toString();
      expected = "    cf_role=timeseries_id\n" +
          "    ioos_category=Identifier\n" +
      ...
      results = table.dataToString();
      expected = "longitude,latitude,altitude,time,line_station,obsScientific,obsValue,obsUnits\n" +
      ...
      table.readNcCF(fileName, StringArray.fromCSV(
          ...
          null, null, null);
      results = table.dataToString();
      expected = "obsScientific,obsValue,obsUnits\n" +
    ...
    // String2.log("\n** Expected errors");
    try {
      table.readNcCF(TableTests.class.getResource("/data/nccf/badIndexed1To6.nc").getPath(), null,
          0, // standardizeWhat=0
          null, null, null);
    ...
    try {
      table.readNcCF(
          TableTests.class.getResource("/largeFiles/kevin/interpolated_gld.20120620_045152_meta_2.nc")
              .getPath(),
          null, 0, // standardizeWhat=0
          null, null, null);
      throw new SimpleException("Shouldn't get here.");
  ...
  void testReadNcCF1Kevin() throws Exception {
    ...
    Table table = new Table();
    String results, expected;
    String fileName = TableTests.class.getResource("/largeFiles/kevin/interpolated_gld.20120620_045152_meta_2.nc")
  ...
  void testReadGocdNcCF() throws Exception {
    ...
    // String2.log(table.toCSVString());
    results = table.dataToString(5);
    expected = "sampling_interval,seafloor_depth,latitude,longitude,latitude_quality_flag,longitude_quality_flag," +
    ...
    // String2.log(table.toCSVString());
    results = table.dataToString(5);
    expected = "sampling_interval,seafloor_depth,latitude,longitude," +
  ...
  void testReadNcCF2() throws Exception {
    ...
    for (int type = 0; type < 2; type++) {
      ...
      String fileName = TableTests.class.getResource("/data/nccf/" +
      ...
      table.readNcCF(fileName, StringArray.fromCSV(
          ...
          null, null, null);
      results = table.dataToString();
      expected = "cruise,org,type,station_id,longitude,latitude,time,platform\n" +
      ...
      results = table.dataToString();
      expected = "salinity,platform,cruise\n" +
      ...
      if (fileType.equals("ragged"))
        expected += "-34.58,173.54,1.33514142E9,\n" +
  ...
  void testReadNcCFASAProfile() throws Exception {
    ...
    /* */
    // *************** profile contiguous
    fileName = TableTests.class.getResource("/data/CFPointConventions/profile/" +
    ...
    results = table.columnAttributes(0).toString();
    expected = "    long_name=station latitude\n" +
    ...
    expected = "lat,lon,profile,time,alt,temperature,humidity,wind_speed\n" +
  ...
  void testReadNcCFASATimeSeries() throws Exception {
    ...
    fileName = TableTests.class.getResource("/data/CFPointConventions/timeSeries/" +
    ...
    expected = "lat,lon,station_name,alt,time,temperature,humidity\n" +
    ...
    expected = "Station-0,8.0,146.0,345600\n" +
    ...
    table.readNcCF(fileName, StringArray.fromCSV("lat,lon,station_name,time,temperature"),
        ...
        StringArray.fromCSV("time,lat"), StringArray.fromCSV("=,="), StringArray.fromCSV("345600,165"));
    results = table.dataToString();
    expected = "lat,lon,station_name,time,temperature\n" +
    ...
    expected = "lat,lon,station_elevation,station_info,station_name,alt,time,temperature,humidity\n" +
  ...
  void testReadNcCFASATrajectory() throws Exception {
    ...
    fileName = TableTests.class.getResource("/data/CFPointConventions/trajectory/" +
    ...
    table.readNcCF(fileName, null, 0, // standardizeWhat=0
        null, null, null);
    results = table.dataToString(20);
    expected = "lat,lon,trajectory_info,trajectory_name,time,z,temperature,humidity\n" +
    ...
    expected = "    coordinates=time lat lon z\n" +
        "    long_name=Air Temperature\n" +
        "    missing_value=-999.9f\n" +
        "    standard_name=air_temperature\n" +
        "    units=Celsius\n";
  ...
  void testReadNcCFASATimeSeriesProfile() throws Exception {
    ...
    Table table = new Table();
    String results, expected, fileName;
    String orthoMultiDimH51FileName = TableTests.class.getResource("/data/CFPointConventions/timeSeriesProfile/" +
        "timeSeriesProfile-Orthogonal-Multidimensional-MultipeStations-H.5.1/" +
        "timeSeriesProfile-Orthogonal-Multidimensional-MultipeStations-H.5.1.nc").getPath();
    String raggedSingleStationFileName = TableTests.class
        .getResource("/data/CFPointConventions/timeSeriesProfile/" +
    ...
    fileName = TableTests.class.getResource("/data/CFPointConventions/timeSeriesProfile/" +
    ...
    table.readNcCF(fileName, null, 0, // standardizeWhat=0
        null, null, null);
    results = table.dataToString(5);
    expected = "lat,lon,station_info,station_name,alt,time,temperature\n" +
    ...
    expected = "lat,lon,station_info,station_name,alt,time,temperature\n" +
        ...
        "37.5,-76.5,0,Station1,10.0,0,0.4\n";
    Test.ensureEqual(results.substring(0, expected.length()), expected, "results=\n" + results);
    expected = "37.5,-76.5,0,Station1,287.5,10800,11.5\n" +
    ...
    table.readNcCF(fileName, null, 0, // standardizeWhat=0
        null, null, null);
    results = table.dataToString(12);
    expected = "lat,lon,alt,station_info,station_name,time,temperature,humidity\n" +
    ...
    table.readNcCF(fileName, null, 0, // standardizeWhat=0
        null, null, null);
    results = table.dataToString();
    expected = "lat,lon,station_info,station_name,profile,time,height,temperature\n" +
        "37.5,-76.5,0,Station1,0,0,0.5,6.7\n" +
        "37.5,-76.5,0,Station1,0,0,1.5,6.9\n" +
    ...
    table.readNcCF(raggedSingleStationFileName,
        ...
        null, null, null);
    results = table.dataToString();
    expected = "station_info,station_name,lon,lat,time,profile\n" +
    ...
    results = table.dataToString();
    expected = "station_info,temperature,station_name\n" +
        "0,8.4,Station1\n" +
        "0,9.2,Station1\n" +
        "0,8.3,Station1\n";
    Test.ensureEqual(results, expected, "results=\n" + results);
    ...
    Test.ensureEqual(table.nRows(), 3, "");
    results = table.columnAttributes(0).toString();
    expected = "    coordinates=time lat lon height\n" +
        "    long_name=Water Temperature\n" +
        "    missing_value=-999.9f\n" +
  ...
  void testReadNcCFASATrajectoryProfile() throws Exception {
    ...
    Table table = new Table();
    String results, expected, fileName;
    String orthoMultiDimH61FileName = TableTests.class.getResource("/data/CFPointConventions/trajectoryProfile/" +
        "trajectoryProfile-Multidimensional-MultipleTrajectories-H.6.1/" +
        "trajectoryProfile-Multidimensional-MultipleTrajectories-H.6.1.nc").getPath();
    String raggedMultipleStationFileName = TableTests.class
        .getResource("/data/CFPointConventions/trajectoryProfile/" +
    ...
    try {
      fileName = TableTests.class.getResource("/data/CFPointConventions/trajectoryProfile/" +
      ...
      table.readNcCF(fileName, null, 0, // standardizeWhat=0
          null, null, null);
      results = table.dataToString();
      expected = "lat,lon,trajectory,alt,time,temperature,salinity\n" +
    ...
    try {
      ...
      table.readNcCF(orthoMultiDimH61FileName, StringArray.fromCSV("lat,lon,trajectory,time,zztop"),
          ...
          StringArray.fromCSV("2"));
      results = table.dataToString();
      expected = "lat,lon,trajectory,time\n" +
    ...
    try {
      ...
      table.readNcCF(fileName, null, 0, // standardizeWhat=0
          null, null, null);
      results = table.dataToString();
      expected = "trajectory,lat,lon,time,z,temperature,humidity\n" +
      ...
      Test.ensureEqual(results.substring(0, expected.length()), expected,
          "results=\n" + results);
      expected = "4,47.0,-44.0,136800,33.0,8.7725,74.930565\n" +
      ...
      Test.ensureEqual(table.nRows(), 20, "");
      results = table.columnAttributes(0).toString();
      expected = "    long_name=time of measurement\n" +
          "    missing_value=-999i\n" +
          "    standard_name=time\n" +
          "    units=seconds since 1990-01-01 00:00:00\n";
      Test.ensureEqual(results, expected, "results=\n" + results);
      ...
      table.readNcCF(raggedMultipleStationFileName,
          ...
          null, null, null);
      results = table.dataToString();
      expected = "lon,lat,time,trajectory\n" +
          "-60.0,49.0,176400,0\n" +
          "-60.0,53.0,104400,0\n" +
          "-63.0,45.0,140400,0\n" +
          "-62.0,45.0,151200,0\n" +
      ...
      Test.ensureEqual(table.nRows(), 20, "");
      results = table.columnAttributes(0).toString();
      expected = "    long_name=station longitude\n" +
      ...
      results = table.dataToString();
      expected = "trajectory,temperature\n" +
          "0,39.174652\n" +
          "0,39.157566\n" +
          "0,39.915646\n" +
          "0,39.041203\n" +
   ...
   * I switched to StringHolder in StringArray after complaints this file took
   * ~10GB to load.
  ...
  void testBigAscii() throws Exception {
    ...
    table.readASCII(TableTests.class.getResource("/largeFiles/biddle/3937_v1_CTD_Profiles.tsv.gz").getPath());
    time = System.currentTimeMillis() - time;
    Math2.gcAndWait("Table (between tests)");
    Math2.gcAndWait("Table (between tests)"); // in a test
    ...
    results = table.saveAsNccsv(false, true, 0, Integer.MAX_VALUE); // catchScalars, writeMetadata, firstDataRow,
                                                                    // lastDataRow
  ...
  /**
   * This tests if nc files are closed when "try with resources" approach is
   * taken.
   * 2022-09-09 With tryWithResources, ERDDAP crashes every 2 days (like THREDDS)!
  ...
  void testNcCloseTryWithResources() throws Throwable {
    ...
    long time = System.currentTimeMillis();
    String fileName = Path.of(TableTests.class
        .getResource("/data/points/erdCalcofiSubsurface/1950/subsurface_19500106_69_144.nc").toURI())
        .toString();
    int n = 100000;
    for (int i = 0; i < n; i++) {
    ...
    String2.log(
        "Opened the same file " + n + " times and auto-closed them. time=" + (System.currentTimeMillis() - time)
            + " (exp=35534ms)");
    ...
    Table table = FileVisitorDNLS.oneStep(
        Path.of(TableTests.class.getResource("/data/points/scrippsGliders/batch14/").toURI()).toString(),
        ".*\\.nc", true, "", false);
    ...
    String2.log(
        "Opened " + n + " files and auto-closed them. time=" + (System.currentTimeMillis() - time)
            + " (exp=4530ms)");
  }
  ...
  void testXml() throws Exception {
    ...
    String stationsXml = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\" ?>\n" +
  ...
  void testSql() throws Exception {
    ...
    if (true) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        timestampDoubles[i] = timestamps[i] == null ? Double.NaN
            : Calendar2.isoStringToEpochSeconds(timestamps[i]);
      }
      ...
      tempTable.addColumn("int",
          PrimitiveArray.factory(new int[] { Math2.random(1000), 0, Integer.MAX_VALUE, 20 }));
  ...
  void testIobis() throws Exception {
    ...
    Table table = new Table();
    if (true) {
      table.readIobis(Table.IOBIS_URL, "Macrocystis", "", // String genus, String species,
          ...
          new String[] { "Institutioncode", "Collectioncode", "Scientificname", "Temperature" }); // String
                                                                                                  // loadColumns[])
  ...
  private static Table getTestTable(boolean includeLongs, boolean includeStrings) {
    ...
    if (includeLongs) {
      ...
      table.columnAttributes(col).set("long_att2",
          new LongArray(new long[] { -2000000000000000L, 2000000000000000L }));
    }
