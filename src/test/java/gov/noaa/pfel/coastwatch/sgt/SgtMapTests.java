...
class SgtMapTests {
        ...
        for (int region = 0; region < 3; region++) { // for all regions, start at 0
          for (int size = (allSizes ? 0 : 1); size < (allSizes ? 3 : 2); size++) {
            ...
            testMakeMap((Graphics2D) oar[0], "over", 0, 0, pdfWidths[size], pdfHeights[size],
                region, 0, SgtMap.PDF_FONTSCALE); // 0=boundaryResAdjust,
  ...
  void testRegionsMap(double minX, double maxX, double minY, double maxY)
    ...
    SgtMap.makeRegionsMap(300, 200, // size
        regionInfo, dir, name);
  ...
  void testCreateTopographyGrid() throws Exception {
    ...
    // 1 point
    grid = SgtMap.createTopographyGrid(null, -10, -10, -80, -80, Integer.MAX_VALUE, Integer.MAX_VALUE);
    ...
    grid = SgtMap.createTopographyGrid(null, -10.002, -10.001, -80.002, -80.001, Integer.MAX_VALUE, Integer.MAX_VALUE);
    Test.ensureEqual(grid.lon.length, 1, "");
    Test.ensureEqual(grid.lon[0], -10, "");
    Test.ensureEqual(grid.lat.length, 1, "");
    Test.ensureEqual(grid.lat[0], -80, "");
    ...
    grid = SgtMap.createTopographyGrid(null, -10.001, -9.97, -80.001, -79.97, Integer.MAX_VALUE, Integer.MAX_VALUE);
    ...
    // 2x2 points, approx lon and lat
    grid = SgtMap.createTopographyGrid(null, -10.001, -9.001, -80.001, -79.001, Integer.MAX_VALUE, Integer.MAX_VALUE);
   ...
   *
   * @param g2
   * @param drawLandMask      One of the non-"" SgtMap.drawLandMask_OPTIONS:
   *                          "under", "over", "outline", or "off"
   * @param baseULXPixel
   * @param baseULYPixel
   * @param imageWidth
   * @param imageHeight
   ...
   */
  void testMakeMap(Graphics2D g2, String drawLandMask,
   ...
   *                          6=PacIsland offset,
   *                          7=-400 to -40, 8=40 to 400, 9=-540 to 540,
   *                          10=C2-360, 11=C2+360
   * @param boundaryResAdjust
   * @param fontScale
   * @throws Exception if trouble
   */
  void testBathymetryMap(boolean bathCpt, Graphics2D g2,
      int baseULXPixel, int baseULYPixel, int imageWidth, int imageHeight,
      int region,
      int boundaryResAdjust, double fontScale) throws Exception {
    ...
    SgtMap.makeMap(false,
        ...
        true, // plotGridData,
        SgtMap.createTopographyGrid(SgtMap.fullPrivateDirectory,
            minX[region], maxX[region], minY[region], maxY[region],
            predicted[0], predicted[1]),
        1, 1, 0, // double gridScaleFactor, gridAltScaleFactor, gridAltOffset,
        bathCpt ? SgtMap.bathymetryCptFullName : SgtMap.topographyCptFullName,
        bathCpt ? SgtMap.BATHYMETRY_BOLD_TITLE + " (" + SgtMap.BATHYMETRY_UNITS + ")"
