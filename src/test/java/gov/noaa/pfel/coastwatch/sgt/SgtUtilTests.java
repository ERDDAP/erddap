...
class SgtUtilTests {
  ...
  void basicTest() throws Exception {
    ...
    Test.ensureEqual(SgtUtil.suggestPaletteRange(.3, 8.9), new double[] { 0, 10 }, ""); // typical Rainbow Linear
    Test.ensureEqual(SgtUtil.suggestPaletteRange(.11, 890), new double[] { .1, 1000 }, ""); // typical Rainbow Log
    Test.ensureEqual(SgtUtil.suggestPaletteRange(-7, 8), new double[] { -10, 10 }, ""); // typical BlueWhiteRed Linear
                                                                                        // symmetric

    // test SgtUtil.suggestPalette
    Test.ensureEqual(SgtUtil.suggestPalette(.3, 8.9), "WhiteRedBlack", ""); // small positive, large positive
    Test.ensureEqual(SgtUtil.suggestPalette(300, 890), "Rainbow", ""); // typical Rainbow Log
    Test.ensureEqual(SgtUtil.suggestPalette(-7, 8), "BlueWhiteRed", ""); // typical BlueWhiteRed Linear symmetric
