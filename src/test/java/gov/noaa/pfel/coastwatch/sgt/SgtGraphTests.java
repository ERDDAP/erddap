...
class SgtGraphTests {
  ...
  void testForMemoryLeak() throws Exception {
    ...
    Math2.gcAndWait("SgtGraph (between tests)"); // in a test, before getMemoryInUse(). Ensure all garbage collected.
    ...
    if (lpr > 0) {
      throw new Exception("memory increase, suspected memory leak: ** SgtGraph test for memory leak: nReps=" + nReps +
          " memoryUsing=" + using + " leak/rep=" + lpr);
    }
  }
  ...
  private static void testDiverseGraphs(boolean testAllAndDisplay, boolean xIsLogAxis, boolean yIsLogAxis)
      throws Exception {
    // verbose = true;
    // reallyVerbose = true;
    // PathCartesianRenderer.verbose = true;
    ...
    PrimitiveArray yCol2 = PrimitiveArray.factory(new double[] { Double.NaN, Double.NaN, Double.NaN, Double.NaN }); // comment
                                                                                                                    // out
                                                                                                                    // for
                                                                                                                    // another
                                                                                                                    // test
    ...
    PrimitiveArray xCol3 = PrimitiveArray
        .factory(new double[] { 1.500e9, 1.501e9, 1.502e9, 1.503e9, 1.504e9, 1.505e9, 1.506e9 });
    ...
    PrimitiveArray xCol5 = PrimitiveArray
        .factory(new double[] { 1.500e9, 1.501e9, 1.502e9, 1.503e9, 1.504e9, 1.505e9, 1.506e9 });
    PrimitiveArray yCol5 = PrimitiveArray
        .factory(new double[] { 1.500e9, 1.504e9, 1.501e9, 1.500e9, 1.503e9, 1.502e9, 1.506e9 });
    ...
    sgtGraph.makeGraph(true, "xAxisTitle", "yAxisTitle",
        ...
        SgtGraph.DefaultBackgroundColor, 1.5); // fontScale
    sgtGraph.makeGraph(false, "xAxisTitle", "yAxisTitle",
        SgtUtil.LEGEND_BELOW, "Graph 2,", "no time, no data.",
        imageDir, "noaa20.gif",
        Double.NaN, Double.NaN, true, false, xIsLogAxis, // isAscending, isTimeAxis, isLog
