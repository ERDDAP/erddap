...
class PersistentTableTests {
  ...
  void basicTest() throws Throwable {
    ...
    File2.delete(name);
    int widths[] = { PersistentTable.BOOLEAN_LENGTH,
        PersistentTable.BYTE_LENGTH, PersistentTable.BINARY_BYTE_LENGTH, PersistentTable.BINARY_CHAR_LENGTH,
        PersistentTable.SHORT_LENGTH, PersistentTable.BINARY_SHORT_LENGTH,
        PersistentTable.INT_LENGTH, PersistentTable.BINARY_INT_LENGTH,
        PersistentTable.LONG_LENGTH, PersistentTable.BINARY_LONG_LENGTH,
        PersistentTable.FLOAT_LENGTH, PersistentTable.BINARY_FLOAT_LENGTH,
        PersistentTable.DOUBLE_LENGTH, PersistentTable.BINARY_DOUBLE_LENGTH, 20 };
    ...
    for (int mode = 0; mode < modes.length; mode++) {
      ...
      pt = new PersistentTable(name, modes[mode],
          new int[] { 80, PersistentTable.BINARY_DOUBLE_LENGTH, PersistentTable.DOUBLE_LENGTH,
              PersistentTable.BINARY_INT_LENGTH, PersistentTable.INT_LENGTH });
      pt.addRows(n);
      if (mode == 1)
        String2.log("*** Note: 2nd rw test uses flush()");
      ...
      String2.log(modes[mode] + " time to write " + n + " binary doubles=" +
          (System.currentTimeMillis() - time) + "   (" +
          new int[] { 45, 158, 969, 1106 }[mode] + "ms)"); // java 17 45,158,969,1106 java 8 31,125,770,772 java 1.6
                                                           // 16,31,1968,1687
      ...
      String2.log(modes[mode] + " time to write " + n + " ints=" +
          (System.currentTimeMillis() - time) + "   (" +
          new int[] { 15, 136, 148, 137 }[mode] + "ms)"); // java 17 15,136,148,137 java 8 16,110,109,106 java 1.6
                                                          // 16,0,219,219
      ...
      String2.log(modes[mode] + " time to write " + n + " binary int=" +
          (System.currentTimeMillis() - time) + "   (" +
          new int[] { 22, 119, 503, 569 }[mode] + "ms)"); // java 17 22,119,503,137 java 8 15,125,360,402 java 1.6
                                                          // 15,1531,922
