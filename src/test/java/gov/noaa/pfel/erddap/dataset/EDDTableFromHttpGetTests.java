...
class EDDTableFromHttpGetTests {
   ...
   *               If hammer==0, this prints the hammer'd data file -- raw, no
   *               processing.
   *               If hammer==3, repeatedly checks the hammer'd data file to
   *               ensure it ends with ']'.
   */
  private static void testStatic(int hammer) throws Throwable {
    ...
    EDDTableFromHttpGet.parseHttpGetDirectoryStructure("5years/3MonTH/4DayS/5hr/6min/7sec/100millis/stationID",
    ...
    // test whichFile
    Test.ensureEqual(
        EDDTableFromHttpGet.whichFile("/ab/", dsColumnName, dsN, dsCalendar,
    ...
    String columnNames[] = { "stationID", "time",
        ...
        EDDTableFromHttpGet.TIMESTAMP, EDDTableFromHttpGet.AUTHOR, EDDTableFromHttpGet.COMMAND };
    ...
    if (hammer == 0) {
      ...
      table = EDDTableFromHttpGet.readFile(name, columnNamesSA, columnTypes,
    ...
    if (hammer > 0) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        results = EDDTableFromHttpGet.insertOrDelete(language, startDir, dsColumnName, dsN, dsCalendar,
    ...
    // read the data
    table = EDDTableFromHttpGet.readFile(startDir + "46088/46088_1980-01.jsonl",
        columnNamesSA, columnTypes, requiredVariableNames, requiredVariableTypes,
        true, Double.NaN);
    results = table.dataToString();
    expected = "stationID,time,aByte,aChar,aShort,anInt,aFloat,aDouble,aString,timestamp,author,command\n" +
    ...
    results = EDDTableFromHttpGet.insertOrDelete(language, startDir, dsColumnName, dsN, dsCalendar,
        keys, tGlobalAttributes,
        columnNames, columnUnits, columnPATypes, columnMvFv,
        requiredVariableNames, EDDTableFromHttpGet.INSERT_COMMAND,
  ...
  void testGenerateDatasetsXml() throws Throwable {
    // testVerboseOn();
    int language = 0;
    String dataDir = Path.of(EDDTestDataset.class.getResource("/data/points/testFromHttpGet/").toURI()).toString()
        + "/";
    String sampleFile = dataDir + "testFromHttpGet.jsonl";

    String results = EDDTableFromHttpGet.generateDatasetsXml(language,
        dataDir, sampleFile,
        "stationID, time",
        "stationID/2months",
   ...
   * Note that Euro is \\u20ac (and low byte is #172 is \\u00ac -- I worked to
   * encode as '?')
   *
  ...
  void testBasic() throws Throwable {
    ...
    expected = "    String httpGetDirectoryStructure \"stationID/2months\";\n" +
    ...
    expected = "\\{\n" +
        ...
        "\\}\n";
    Test.ensureLinesMatch(results, expected, ""); // sometimes fails if right at change of hour (to "today" is previous
                                                  // hour)
    ...
    Math2.sleep(1);
    String versioningExpected = "stationID,time,latitude,longitude,airTemp,waterTemp,timestamp,author,command\n" +
    ...
    results = File2.directReadFromUtf8File(
        Path.of(
            EDDTestDataset.class.getResource("/data/points/testFromHttpGet/station2/station2_2016-05.jsonl").toURI())
            .toString());
    // String2.log(results);
    expected = "\\[\"stationID\",\"time\",\"airTemp\",\"waterTemp\",\"timestamp\",\"author\",\"command\"\\]\n" +
  ...
  /**
   * Run TestAll (set to call this) a few times simultaneously and enter 1, 2, 3,
   * 4.
   * '3' repeatedly tests that the file is valid.
   * Wait till they are done, then run this with 0 to see resulting file from
   * multithreaded hammer test.
   */
  @TagIncompleteTest
  @org.junit.jupiter.api.Test
