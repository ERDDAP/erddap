...
class EDDTableFromFileNamesTests {
  ...
  void testGenerateDatasetsXml() throws Throwable {
    ...
    String tDir = Path.of(EDDTableFromFileNamesTests.class.getResource("/data/fileNames/").toURI()).toString();
    tDir = tDir.replace('\\', '/');
    String tRegex = ".*\\.png";
    ...
    String expected = "<dataset type=\"EDDTableFromFileNames\" datasetID=\"" + tDatasetID + "\" active=\"true\">\n" +
        ...
        "    -->\n" +
        "</dataset>\n\n\n";
    String results = EDDTableFromFileNames.generateDatasetsXml(tDir, tRegex, tRecursive,
  ...
  void testLocal() throws Throwable {
    ...
    expected = "five,fixedTime,latitude,longitude,mySpecialString,fvEmptyString\n" +
  ...
  void testAwsS3() throws Throwable {
    ...
    tName = tedd.makeNewFileForDapQuery(language, null, null, "", dir,
        tedd.className() + "_all", ".csv");
    results = File2.directReadFrom88591File(dir + tName);
    expected = "five,url,name,startMonth,endMonth,lastModified,size,fileType\n" +
        "m,,,UTC,UTC,UTC,bytes,\n" +
   ...
   */
  private static void testAccessibleViaFilesFileTable(boolean deleteCachedInfo,
  ...
  void testAwsS3local() throws Throwable {
    ...
    expected = "Description\tPopulation Code\n" +
        "East Asian\tEAS\n" +
        "South Asian\tSAS\n" +
        "African\tAFR\n" +
        "European\tEUR\n" +
  ...
  void testOnTheFly() throws Throwable {
    ...
    results = File2.directReadFrom88591File(dir + tName);
    expected = "url,name,lastModified,size,fileType\n" +
        ",,UTC,bytes,\n" +
