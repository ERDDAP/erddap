...
// None of the thests in this class currently run. They were all disabled before the JUnit migration.
// Tests were migrated over and not changed. They all rely on SOS server and all SOS tests were previously disabled.
class EDDTableTests {
  /**
  ...
  void testSosNdbcMet() throws Throwable {
    ...
    String sosQuery1 = "service=SOS&version=1.0.0&request=GetObservation" +
    ...
    String sosQuery1Csv = "service=SOS&version=1.0.0&request=GetObservation" +
    ...
    eddTable.sosGetObservation(language, endOfRequest, sosQuery1Csv, "someIPAddress", null, osss, dir,
        "testSos1Sta");
    ...
    // response
    String sosQuery1b = "service=SOS&version=1.0.0&request=GetObservation" +
        "&offering=urn:ioos:Station:1.0.0.127.cwwcNDBCMet:41004:" +
        "&observedProperty=cwwcNDBCMet" +
        "&responseFormat=text/csv";
    ...
    String sosQuery2 = "service=SOS&version=1.0.0&request=GetObservation" +
    ...
    results = baos.toString(File2.UTF_8);
    String2.log(results);
    expected = "longitude, latitude, time, station, wd, wspd, gst, wvht, dpd, apd, mwd, bar, atmp, wtmp, dewp, vis, ptdy, tide, wspu, wspv\n"
        +
        "degrees_east, degrees_north, UTC, , degrees_true, m s-1, m s-1, m, s, s, degrees_true, hPa, degree_C, degree_C, degree_C, km, hPa, m, m s-1, m s-1\n"
        +
        "-79.09, 32.5, 2008-08-01T00:00:00Z, 41004, 225, 10.9, 14.0, 1.66, 5.26, 4.17, NaN, 1007.6, 27.8, 27.9, NaN, NaN, NaN, NaN, 7.7, 7.7\n"
    ...
    String sosQuery2b = "service=SOS&version=1.0.0&request=GetObservation" +
        "&offering=urn:ioos:network:1.0.0.127.cwwcNDBCMet:cwwcNDBCMet" +
    ...
    results = baos.toString(File2.UTF_8);
    String2.log(results);
    expected = "longitude, latitude, time, station, atmp, wtmp\n" +
    ...
    String sosQuery2c = "service=SOS&version=1.0.0&request=GetObservation" +
        "&offering=urn:ioos:network:1.0.0.127.cwwcNDBCMet:cwwcNDBCMet" +
        "&observedProperty=atmp," + fullPhenomenaDictionaryUrl + "#wtmp" + // short and long
        "&responseFormat=text/xml;schema=%22ioos/0.6.1%22" +
        "&eventTime=2008-08-01T00:00:00Z/2008-08-01T01:00:00Z" +
    ...
    // stations)
    // featureOfInterest=BBOX:<min_lon>,<min_lat>,<max_lon>,<max_lat>
    String sosQuery3 = "service=SOS&version=1.0.0&request=GetObservation" +
    ...
    String sosQuery5csv = "service=SOS&version=1.0.0&request=GetObservation" +
    ...
    String sosQuery5png = "service=SOS&version=1.0.0&request=GetObservation" +
        "&offering=41004" + // short
    ...
    baos = new ByteArrayOutputStream();
    osss = new OutputStreamSourceSimple(baos);
    eddTable.sosGetObservation(language, endOfRequest, sosQuery5csv, "someIPAddress", null, osss, dir,
        "testSos5csv");
    results = baos.toString(File2.UTF_8);
    String2.log(results);
    expected = "longitude, latitude, time, station, wtmp\n" +
    ...
    expected = "-79.09, 32.5, 2008-07-31T21:00:00Z, 41004, 28.0\n" +
    ...
    baos = new ByteArrayOutputStream();
    osss = new OutputStreamSourceSimple(baos);
    eddTable.sosGetObservation(language, endOfRequest, sosQuery6csv, "someIPAddress", null, osss, dir,
        "testSos6csv");
    results = baos.toString(File2.UTF_8);
    String2.log(results);
    expected = "longitude, latitude, time, station, wtmp\n" +
    ...
    Test.ensureEqual(results.substring(0, expected.length()), expected, "\nresults=\n" + results);
    expected = "-76.48, 37.23, 2008-08-01T00:00:00Z, YKTV2, 28.2\n" +
  ...
  void testSosCurrents() throws Throwable {
    ...
    baos = new ByteArrayOutputStream();
    osss = new OutputStreamSourceSimple(baos);
    eddTable.sosGetObservation(language, endOfRequest, sosQuery1, "someIPAddress", null, osss, dir, "testSosCurBB");
    /*
    ...
    results = baos.toString(File2.UTF_8);
    // String2.log(results);
    expected = "longitude, latitude, station_id, altitude, time, CurrentDirection, CurrentSpeed\n" +
        "degrees_east, degrees_north, , m, UTC, degrees_true, cm s-1\n" +
    ...
    baos = new ByteArrayOutputStream();
    osss = new OutputStreamSourceSimple(baos);
    eddTable.sosGetObservation(language, endOfRequest, sosQuery2, "someIPAddress", null, osss, dir,
        "testSosCurSta");
    results = baos.toString(File2.UTF_8);
    String2.log(results);
    // expected = same data
    ...
    baos = new ByteArrayOutputStream();
    osss = new OutputStreamSourceSimple(baos);
    eddTable.sosGetObservation(language, endOfRequest, sosQuery3, "someIPAddress", null, osss, dir,
        "testSosCurSta2");
    results = baos.toString(File2.UTF_8);
    // String2.log(results);
    expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
   ...
   * This caused an internal error in EDDTableFromSos. So this helped me fix the
   * bug.
   */
  @org.junit.jupiter.api.Test
  @TagExternalOther
