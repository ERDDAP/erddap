...
class EDDTableFromHyraxFilesTests {
  ...
  void testGenerateDatasetsXml() throws Throwable {
    ...
    String results = EDDTableFromHyraxFiles.generateDatasetsXml(
    ...
    String expected = "<dataset type=\"EDDTableFromHyraxFiles\" datasetID=\"nasa_jpl_6965_9def_b894\" active=\"true\">\n"
        +
   ...
   * This is a bizarre test since it is really gridded data. But no alternatives
   * currently.
   * Actually, this is useful, because it tests serving gridded data via tabledap.
   *
   * <p>
   * Also, this is a test of ERDDAP adjusting valid_range by scale_factor
   * add_offset
   *
  ...
  @TagIncompleteTest // This fails because source was .gz so created local files were called .gz even
  ...
  void testJpl(boolean deleteCachedInfoAndOneFile) throws Throwable {
    ...
    try {
      ...
      if (deleteCachedInfoAndOneFile) {
        ...
        Math2.sleep(5000);
        eddTable = (EDDTable) EDDTestDataset.gettestEDDTableFromHyraxFiles(); // redownload the dataset
      }
    } catch (Throwable t) {
    ...
    try {
      ...
      expected = "tabledap/testEDDTableFromHyraxFiles.das\";\n" +
    ...
    results = File2.directReadFrom88591File(EDStatic.fullTestCacheDirectory + tName);
    // String2.log(results);
    expected = "longitude,latitude,time,uwnd,vwnd,wspd,upstr,vpstr,nobs\n" +
    ...
    results = File2.directReadFrom88591File(EDStatic.fullTestCacheDirectory + tName);
    expected = "longitude,latitude,time,upstr,vpstr\n" +
