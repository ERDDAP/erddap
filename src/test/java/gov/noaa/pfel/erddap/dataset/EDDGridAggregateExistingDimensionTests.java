...
class EDDGridAggregateExistingDimensionTests {
  ...
  void testBasic() throws Throwable {
    ...
    // String2.log(results);
    expected = "time,latitude,longitude,wind_speed\n" +
    ...
    expected = "netcdf EDDGridAggregateExistingDimension.nc {\n" +
        "  dimensions:\n" +
    ...
    expected = "/griddap/ndbcCWind41002.nc?wind_speed[1:5][0][0]\";\n" +
    ...
    results = File2.directReadFrom88591File(EDStatic.fullTestCacheDirectory + tName);
    // String2.log(results);
    expected = "time,latitude,longitude,wind_direction\n" +
    ...
    results = File2.directReadFrom88591File(EDStatic.fullTestCacheDirectory + tName);
    // String2.log(results);
    expected = "time,latitude,longitude,wind_speed,wind_direction\n" +
    ...
    results = File2.directReadFrom88591File(EDStatic.fullTestCacheDirectory + tName);
    // String2.log(results);
    expected = "time,latitude,longitude,wind_direction,wind_speed\n" +
  ...
  /**
   * This tests rtofs - where the aggregated dimension, time, has sourceValues !=
   * destValues
   * since sourceTimeFormat isn't "seconds since 1970-01-01".
   * !!!THIS USED TO WORK, BUT NEEDS TO BE UPDATED; datasets are removed after ~1
   * month
   */
  @org.junit.jupiter.api.Test
  @TagMissingDataset
  void testRtofs() throws Throwable {
    ...
    int language = 0;
    EDDGrid eddGrid = (EDDGrid) EDDGridAggregateExistingDimension.oneFromDatasetsXml(null, "RTOFSWOC1");
  ...
  void testFiles() throws Throwable {
    ...
    try {
      ...
      expected = "java.io.IOException: HTTP status code=404 java.io.FileNotFoundException: http://localhost:8080/cwexperimental/files/nceiOisst2Agg/gibberish/\n"
          +
          "(Error {\n" +
          "    code=404;\n" +
      ...
      expected = "java.io.IOException: HTTP status code=404 java.io.FileNotFoundException: http://localhost:8080/cwexperimental/files/nceiOisst2Agg/gibberish.csv\n"
          +
          "(Error {\n" +
          "    code=404;\n" +
      ...
      expected = "java.io.IOException: HTTP status code=404 java.io.FileNotFoundException: http://localhost:8080/cwexperimental/files/nceiOisst2Agg/subdir/gibberish.csv\n"
          +
          "(Error {\n" +
          "    code=404;\n" +
          "    message=\"Not Found: File not found: gibberish.csv .\";\n" +
  ...
  */
  ...
  void testGenerateDatasetsXml() throws Throwable {
    ...
    results = EDDGridAggregateExistingDimension.generateDatasetsXml("thredds",
        "https://thredds1.pfeg.noaa.gov/thredds/catalog/Satellite/aggregsatMH/chla/catalog.xml",
        ".*", 1440); // recursive
    expected = "<dataset type=\"EDDGridAggregateExistingDimension\" datasetID=\"noaa_pfeg_56a4_10ee_2221\" active=\"true\">\n"
        +
        "\n" +
    ...
    // ****** test HYRAX
    results = EDDGridAggregateExistingDimension.generateDatasetsXml("hyrax",
        ...
        1440);
    expected = "<dataset type=\"EDDGridAggregateExistingDimension\" datasetID=\"nasa_jpl_2458_4e43_c52a\" active=\"true\">\n"
        +
        "\n" +
  ...
  void testGetDodsIndexUrls() throws Exception {
    ...
    try {
      ...
      EDDGridAggregateExistingDimension.getDodsIndexUrls(dir, "[0-9]{8}\\.bodas_ts\\.nc", true, "", sourceUrls);
