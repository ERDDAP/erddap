...
class EDDTableFromDatabaseTests {
   ...
   * pgAdmin3
   * <br>
   * Log in: Double click on "Servers : PostgreSQL 9.3 (localhost:5432)
   * <br>
   * Work in database=postgres schema=public
   * <br>
   * Create table, columns, PrimaryKey: Edit : Create : ...
   * <br>
   * table=mytable
   * 
  ...
  void testGenerateDatasetsXml() throws Throwable {
    ...
    password = "MyPassword";
    String connectionProps[] = new String[] { "user", EDDTableFromDatabase.testUser, "password", password };
    String dir = EDStatic.fullTestCacheDirectory;
    int language = 0;

    // list catalogs
    String2.log("* list catalogs");
    results = EDDTableFromDatabase.getDatabaseInfo(EDDTableFromDatabase.testUrl, EDDTableFromDatabase.testDriver,
        connectionProps,
    ...
    expected = "TABLE_CAT,TABLE_SCHEM,TABLE_NAME,TABLE_TYPE,REMARKS,TYPE_CAT,TYPE_SCHEM,TYPE_NAME,SELF_REFERENCING_COL_NAME,REF_GENERATION\n"
        +
        ",,,,,,,,,\n" +
        ",myschema,id,INDEX,,,,,,\n" +
    ...
    expected = "Col Key Name                    java.sql.Types Java Type Remarks\n" +
        "0   P   id                      4              int       \n" +
        "1       first                   12             String    \n" +
        "2       last                    12             String    \n" +
        "3       height_cm               4              int       \n" +
    ...
    expected = "<!-- NOTE! Since database tables don't have any metadata, you must add metadata\n" +
        "  below, notably 'units' for each of the dataVariables. -->\n" +
        "<dataset type=\"EDDTableFromDatabase\" datasetID=\"myschema_mytable\" active=\"true\">\n" +
        "    <sourceUrl>" + EDDTableFromDatabase.testUrl + "</sourceUrl>\n" +
        "    <driverName>" + EDDTableFromDatabase.testDriver + "</driverName>\n" +
    ...
    expected = "id,first,last,height_cm,weight_kg,time,category\n" +
        ",,,,,UTC,\n" +
   ...
   *
   * @param tDatasetID testMyDatabaseNo or testMyDatabasePartial or
   *                   testMyDatabaseYes,
   *                   to test sourceCanOrderBy=x and sourceCanDoDistinct=x.
   *                   tDatasetID=testMyDabaseYes tests sourceCanOrderBy=yes and
   *                   sourceCanDoDistinct=yes.
   * @throws Throwable if trouble
   */
  @ParameterizedTest
  @TagMissingDataset
  ...
  void testBasic(String tDatasetID) throws Throwable {
    ...
    EDDTableFromDatabase tedd = (EDDTableFromDatabase) EDDTableFromDatabase.oneFromDatasetsXml(null,
        tDatasetID);
    ...
    results = File2.directReadFrom88591File(dir + tName);
    expected = "category,first,last,height,weight_kg,weight_lb,time\n" +
        ",,,cm,kg,lb,UTC\n" +
  ...
  void testNonExistentVariable() throws Throwable {
    ...
    try {
      ...
      EDDTableFromDatabase edd = (EDDTableFromDatabase) EDDTableFromDatabase.oneFromDatasetsXml(null,
