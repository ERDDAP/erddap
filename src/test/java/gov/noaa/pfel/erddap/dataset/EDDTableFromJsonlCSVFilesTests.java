...
class EDDTableFromJsonlCSVFilesTests {
  ...
  void testBasic(boolean deleteCachedDatasetInfo) throws Throwable {
    ...
    if (deleteCachedDatasetInfo)
      EDDTableFromJsonlCSVFiles.deleteCachedDatasetInfo(id);
    ...
    expected = "/erddap/tabledap/testJsonlCSV.das\";\n" +
    ...
    results = File2.directReadFrom88591File(dir + tName);
    // String2.log(results);
    expected = "ship,time,latitude,longitude,status,testLong,sst\n" +
  ...
  /**
   * This tests having base64 image data and displaying it in an
   * .htmlTableResponse.
   *
   * @throws Throwable if trouble
   */
  ...
  void testBase64Image() throws Throwable {
    ...
    tName = eddTable.makeNewFileForDapQuery(language, null, null, "", dir,
        eddTable.className() + "_testBase64Image", ".htmlTable");
    results = File2.directReadFrom88591File(dir + tName);
    // String2.log(results);
    results = results.replaceAll("Version \\d\\.\\d\\d", "Version x.xx");
    results = String2.replaceAll(results, // change favicon, noaab.png and css references to coastwatch erddap so page
                                          // displays well in browser
        "http://127.0.0.1:8080/cwexperimental/",
        "https://coastwatch.pfeg.noaa.gov/erddap/");
    results = String2.replaceAll(results,
        "http&#x3a;&#x2f;&#x2f;127&#x2e;0&#x2e;0&#x2e;1&#x3a;8080&#x2f;cwexperimental", // '?' img file reference
        "https://coastwatch.pfeg.noaa.gov/erddap");
