...
class EDDTableFromAsciiServiceNOSTests {
   ...
   * is imperfect (see above; e.g., it never mentions STND which most, not all,
   * support).
   ...
   * For the soap services, there is no RESTfull (URL) access. So harder to work
   * with.
   ...
   *
   * @param idRegex is a regex indicating which datasetID's should be tested (".*"
   *                for all)
   */
  @ParameterizedTest
  @ValueSource(strings = {
  ...
  void testNosCoops(String idRegex) throws Throwable {
    ...
    if ("nosCoopsWLR6".matches(idRegex)) {
      ...
      expected = // this changes every day
          ...
          "stationID,stationName,state,dateEstablished,shefID,deployment,longitude,latitude,time,datum,dcp,sensor,waterLevel,sigma,O,F,R,L\n"
              ...
              "9414290,San Francisco,CA,1854-06-30T00:00:00Z,FTPC1,\"NWLON,PORTS\",-122.4659,37.8063," + yesterday
    ...
    if ("nosCoopsWLV6".matches(idRegex)) {
      ...
      expected = // changes every day
          ...
          "stationID,stationName,state,dateEstablished,shefID,deployment,longitude,latitude,time,datum,waterLevel,sigma,I,F,R,L\n"
              ...
              "9414290,San Francisco,CA,1854-06-30T00:00:00Z,FTPC1,\"NWLON,PORTS\",-122.4659,37.8063," + daysAgo
    ...
    if ("nosCoopsWLR60".matches(idRegex)) {
      ...
      expected =
          ...
          "stationID,stationName,state,dateEstablished,shefID,deployment,longitude,latitude,time,datum,waterLevel,sigma,I,L\n"
              +
              ",,,UTC,,,degrees_east,degrees_north,UTC,,m,m,,\n" +
              "8454000,Providence,RI,1938-06-03T00:00:00Z,FOXR1,\"NWLON,PORTS\",-71.4012,41.8071," + daysAgo
    ...
    if ("nosCoopsWLVHL".matches(idRegex)) {
      ...
      expected = "stationID,stationName,state,dateEstablished,shefID,deployment,longitude,latitude,time,datum,waterLevel,type,I,L\n"
          +
          ",,,UTC,,,degrees_east,degrees_north,UTC,,m,,,\n";
    ...
    if ("nosCoopsWLTPHL".matches(idRegex)) {
      ...
      // number of lines in results varies
      String sar[] = String2.split(results, '\n');
      expected = "stationID,stationName,state,dateEstablished,shefID,deployment,longitude,latitude,time,datum,waterLevel,type\n"
          +
          ",,,UTC,,,degrees_east,degrees_north,UTC,,m,\n";
      for (int i = 2; i < sar.length - 1; i++)
        expected +=
            ...
            // 2020-04-27 was -71.4006,41.8067
            "8454000,Providence,RI,1938-06-03T00:00:00Z,FOXR1,\"NWLON,PORTS\",-71.4012,41.8071," + daysAhead
    ...
    if ("nosCoopsMRF".matches(idRegex)) {
      ...
      expected =
          ...
          "stationID,stationName,state,dateEstablished,shefID,deployment,longitude,latitude,time,dcp,sensor,RF,X,R\n"
              ...
              + daysAgo + "00:00:00Z,1,J1,\\d\\.\\d,0,(0|1)\n" +
              "9752619,\"Isabel Segunda, Vieques Island\",PR,2007-09-13T00:00:00Z,VQSP4,COASTAL,-65.4439,18.1525,"
              + daysAgo + "00:06:00Z,1,J1,\\d\\.\\d,0,(0|1)\n" +
              "9752619,\"Isabel Segunda, Vieques Island\",PR,2007-09-13T00:00:00Z,VQSP4,COASTAL,-65.4439,18.1525,"
              + daysAgo + "00:12:00Z,1,J1,\\d\\.\\d,0,(0|1)\n" +
              "9752619,\"Isabel Segunda, Vieques Island\",PR,2007-09-13T00:00:00Z,VQSP4,COASTAL,-65.4439,18.1525,"
              + daysAgo + "00:18:00Z,1,J1,\\d\\.\\d,0,(0|1)\n" +
              "9752619,\"Isabel Segunda, Vieques Island\",PR,2007-09-13T00:00:00Z,VQSP4,COASTAL,-65.4439,18.1525,"
              + daysAgo + "00:24:00Z,1,J1,\\d\\.\\d,0,(0|1)\n" +
              "9752619,\"Isabel Segunda, Vieques Island\",PR,2007-09-13T00:00:00Z,VQSP4,COASTAL,-65.4439,18.1525,"
              + daysAgo + "00:30:00Z,1,J1,\\d\\.\\d,0,(0|1)\n" +
              "9752619,\"Isabel Segunda, Vieques Island\",PR,2007-09-13T00:00:00Z,VQSP4,COASTAL,-65.4439,18.1525,"
              + daysAgo + "00:36:00Z,1,J1,\\d\\.\\d,0,(0|1)\n" +
              "9752619,\"Isabel Segunda, Vieques Island\",PR,2007-09-13T00:00:00Z,VQSP4,COASTAL,-65.4439,18.1525,"
              + daysAgo + "00:42:00Z,1,J1,\\d\\.\\d,0,(0|1)\n" +
              "9752619,\"Isabel Segunda, Vieques Island\",PR,2007-09-13T00:00:00Z,VQSP4,COASTAL,-65.4439,18.1525,"
              + daysAgo + "00:48:00Z,1,J1,\\d\\.\\d,0,(0|1)\n" +
              "9752619,\"Isabel Segunda, Vieques Island\",PR,2007-09-13T00:00:00Z,VQSP4,COASTAL,-65.4439,18.1525,"
              + daysAgo + "00:54:00Z,1,J1,\\d\\.\\d,0,(0|1)\n";// +
      Test.ensureLinesMatch(results, expected, "results=\n" + results);
    }
    ...
    if ("nosCoopsMRH".matches(idRegex)) {
      ...
      expected =
          ...
          "stationID,stationName,state,dateEstablished,shefID,deployment,longitude,latitude,time,dcp,sensor,RH,X,N,R\n"
              ...
              "9063063,Cleveland,OH,1860-01-01T00:00:00Z,CNDO1,NWLON,-81.6355,41.5409," + daysAgo
    ...
    if ("nosCoopsMV".matches(idRegex)) {
      ...
      tName = edd.makeNewFileForDapQuery(language, null, null, query, EDStatic.fullTestCacheDirectory,
          edd.className() + "_" + edd.datasetID(), ".csv");
      results = File2.directReadFrom88591File(EDStatic.fullTestCacheDirectory + tName);
      expected = "stationID,stationName,state,dateEstablished,shefID,deployment,longitude,latitude,time,Vis\n" +
      ...
      for (int i = 2; i < sar.length - 1; i++)
        expected +=
            ...
            "8737005,Pinto Island Visibility,AL,2009-12-15T00:00:00Z,PTOA1,PORTS,-88.031,30.6712," + daysAgo
                + "..:..:00Z,([\\-\\.\\d]{1,6}|NaN)\n";
      Test.ensureLinesMatch(results, expected, "results=\n" + results);
    }
