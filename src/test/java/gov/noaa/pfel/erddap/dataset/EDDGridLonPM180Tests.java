...
class EDDGridLonPM180Tests {
  ...
  void testGenerateDatasetsXmlFromErddapCatalog() throws Throwable {
    ...
    // ***\n");
    // testVerboseOn();
    String url = "http://coastwatch.pfeg.noaa.gov/erddap/"; // purposefully http:// to test if ERDDAP will promote
                                                            // to https://
    ...
    String expected = "<dataset type=\"EDDGridLonPM180\" datasetID=\"erdMBsstdmday_LonPM180\" active=\"true\">\n" +
  ...
  void testGT180() throws Throwable {
    ...
    EDDGrid eddGrid = (EDDGrid) EDDGridLonPM180.oneFromDatasetsXml(null, "test_erdMWchlamday_LonPM180");
    ...
    expected = "    Float64 geospatial_lon_max -105.0;\n" +
    ...
    expected = "longitude, degrees_east, -155.0, -150.0, -145.0, -140.0, -135.0, -130.0, -125.0, -120.0, -115.0, -110.0, -105.0, ";
    Test.ensureEqual(results, expected, "results=\n" + results);
  ...
  void test1to359() throws Throwable {
    ...
    expected = "    Float64 geospatial_lon_max 179.75;\n" +
    ...
    results = File2.directReadFrom88591File(dir + tName);
    results = String2.replaceAll(results, "\n", ", ");
    expected = "longitude, degrees_east, -179.75, -129.75, -79.75, -29.75, 20.25, 70.25, 120.25, 170.25, ";
    Test.ensureEqual(results, expected, "results=\n" + results);
    ...
    results = File2.directReadFrom88591File(dir + tName);
    // String2.log(results);
    expected = "time,altitude,latitude,longitude,dhw\n" +
  ...
  void test0to360() throws Throwable {
    ...
    expected = "    Float64 geospatial_lon_max 179.97916425512213;\n" +
    ...
    results = File2.directReadFrom88591File(dir + tName);
    results = String2.replaceAll(results, "\n", ", ");
    expected = "longitude, degrees_east, -179.97916425512213, -128.55654589651581, -77.13392753790947, -25.711309179303157, 25.71130917930316, 77.13392753790947, 128.5565458965158, 179.97916425512213, ";
    Test.ensureEqual(results, expected, "results=\n" + results);
  ...
  void testFiles() throws Throwable {
    ...
    results = String2.annotatedString(SSR.getUrlResponseStringNewline(
        ...
        .substring(0, 50));
    expected = "[31][139][8][8] [26]eO[0][3]MW2002182_2002212_chla.nc[0][228][216][127][140]#[231]][199]q[223][229][238]r?[end]";
    Test.ensureEqual(results, expected, "results=\n" + results);
    ...
    expected = "java.io.IOException: HTTP status code=404 java.io.FileNotFoundException: http://localhost:8080/cwexperimental/files/local_erdMWchlamday_LonPM180/gibberish/\n"
        +
        "(Error {\n" +
        "    code=404;\n" +
    ...
    expected = "java.io.IOException: HTTP status code=404 java.io.FileNotFoundException: http://localhost:8080/cwexperimental/files/local_erdMWchlamday_LonPM180/gibberish.csv\n"
        +
        "(Error {\n" +
        "    code=404;\n" +
