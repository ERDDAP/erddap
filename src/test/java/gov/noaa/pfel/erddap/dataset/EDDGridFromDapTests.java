...
class EDDGridFromDapTests {
    ...
    void testBasic2() throws Throwable {
        ...
        results = String2
                .annotatedString(new String((ByteArray.fromFile(EDStatic.fullTestCacheDirectory + tName)).toArray()));
        ...
        expected = EDStatic.startHeadHtml(language, EDStatic.erddapUrl((String) null, language),
                ...
                + "\n" +
                "</head>\n" +
                EDStatic.startBodyHtml(language, null, "griddap/erdMHchla8day.html", tQuery) + // 2022-11-22 .html
                                                                                               // because
                                                                                               // language selector
                                                                                               // system here
                                                                                               // changes .htmlTable to
                                                                                               // .html to
                                                                                               // avoid
                "&nbsp;<br>\n" +
                "&nbsp;\n" +
    ...
    void testBasic3() throws Throwable {
        ...
        tName = gridDataset.makeNewFileForDapQuery(language, null, null, userDapQuery,
                EDStatic.fullTestCacheDirectory, gridDataset.className() + "_Map", // must be Map because .asc already
                                                                                   // used
                ".esriAscii");
        // Test.displayInBrowser("file://" + EDStatic.fullTestCacheDirectory + tName);
        results = File2.directReadFrom88591File(EDStatic.fullTestCacheDirectory + tName);
    ...
    void testGeotif2() throws Throwable {
        ...
        Test.displayInBrowser("file://" + tDir + tName); // doesn't display it or show error message. //geotiff, so
                                                         // don't
    ...
    void testOpendap() throws Throwable {
        ...
        Test.ensureEqual(attributes.get("fraction_digits"), new IntArray(new int[] { 4 }), ""); // test if stored in
                                                                                                // correct
                                                                                                // form
        ...
        String2.log("\nFrom erddap:\n" + String2.annotatedString(
                SSR.getUrlResponseStringUnchanged(erddapUrl + ".asc?" + erddapQuery))); // in tests, always non-https
                                                                                        // url
        ...
        String2.log("\nFrom erddap:\n" + String2.annotatedString(
                SSR.getUrlResponseStringUnchanged(erddapUrl + ".asc?" + griddapUserDapQuery))); // in tests, always
                                                                                                // non-https
                                                                                                // url
        ...
        NetcdfFile nc = NetcdfDatasets.openFile(EDStatic.erddapUrl + "/griddap/erdMHchla8day", null); // in tests,
                                                                                                      // always
                                                                                                      // non-https url
        try {
            results = nc.toString();
        ...
        EDDGrid eddGrid2 = new EDDGridFromDap(
                ...
                erddapUrl, -1, true); // sourceUrl, nThreads, dimensionValuesInMemory); //in tests, always non-https
                                      // url
    ...
    void testGenerateDatasetsXml() throws Throwable {
        ...
        EDD edd = EDDGridFromDap.oneFromXmlFragment(null, results); // only returns the first dataset defined in results
        ...
        try {
            ...
            expected1 = "<dataset type=\"EDDGridFromDap\" datasetID=\"hawaii_soest_82a3_7247_c8d7\" active=\"true\">\n"
                    ...
                    "        <att name=\"creator_url\">https://www.atmos.umd.edu/~ocean/</att>\n" +
                    "        <att name=\"infoUrl\">http://apdrc.soest.hawaii.edu/datadoc/soda_2.2.4.php</att>\n" +
                    "        <att name=\"institution\">HAWAII SOEST</att>\n" + // some changes to keywords in this test
                                                                               // vs above
                                                                               // test
    ...
    void testGenerateDatasetsXml2() throws Throwable {
        ...
        String expected1 = "<dataset type=\"EDDGridFromDap\" datasetID=\"noaa_pfeg_cada_f1d6_7111\" active=\"true\">\n"
                ...
                "    <reloadEveryNMinutes>[VARIES]</reloadEveryNMinutes>\n" + // 60, 180, or ???, test of
                                                                              // suggestReloadEveryNMinutes, but varies
                                                                              // with
    ...
    void testOneTime() throws Throwable {
        ...
        // soda
        EDDGrid gridDataset = (EDDGridFromDap) EDDGridFromDap.oneFromDatasetsXml(null, "erdSoda202d"); // should work
        String tName = gridDataset.makeNewFileForDapQuery(language, null, null, "",
    ...
    void testDescendingLat() throws Throwable {
        ...
        String query180stride = SSR
                .fixPercentEncodedUrl("topo[0:20:last][0:20:last]&.draw=surface&.vars=longitude|latitude|topo");
        int language = 0;
        /* */
    ...
    void testKml() throws Throwable {
        ...
        try {
            ...
            EDDGridFromDap gridDataset = (EDDGridFromDap) EDDGridFromDap.oneFromDatasetsXml(null, "erdBAssta5day");
            String name, tName, results, expected;
            ...
            expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                    ...
                    "</description>\n" +
                    "    <name>Logo</name>\n" +
                    "    <Icon><href>" + EDStatic.preferredErddapUrl + "/" + EDStatic.IMAGES_DIR + // in tests, always
                                                                                                   // non-https
                                                                                                   // url
                    "nlogo.gif</href></Icon>\n" +
    ...
    void testClimatologyTime() throws Throwable {
        ...
        try {
            EDDGrid eddGrid = (EDDGrid) EDDGridFromDap.oneFromDatasetsXml(null, "ncdcOwClm9505"); // 2020-08-20 this now
                                                                                                  // often fails with
     ...
     */
    @org.junit.jupiter.api.Test
    @TagLargeFile // There's an alternative dataset definition that uses THREDDS.
    void testValidMinMax() throws Throwable {
        ...
        expected = "  sea_surface_temperature {\n" +
                "    Float32 _FillValue -327.68;\n" + // 2020-01-21 appeared with netcdf-java 5.2, so what did this test
                ...
                "    String units \"degree_C\";\n" +
                "    Float32 valid_max 45.0;\n" + // 2020-01-21 vmin/max appeared with netcdf-java 5.2, so what did this
                                                  // test
                                                  // actually test before?
    ...
    void testGenerateDatasetsXml4() throws Throwable {
        ...
        String expected = "<dataset type=\"EDDGridFromDap\" datasetID=\"" + tDatasetID + "\" active=\"true\">\n" +
                ...
                "            <att name=\"units\">kelvin</att>\n" +
                "            <att name=\"valid_max\" type=\"byte\">127</att>\n" +
                "            <att name=\"valid_min\" type=\"short\">-127</att>\n" + // source is short! to deal with dap
                                                                                    // byte
                                                                                    // being 0..255
                "        </sourceAttributes -->\n" +
                "        <addAttributes>\n" +
    ...
    void testGenerateDatasetsXml5() throws Throwable {
        ...
        expected = "<att name=\"easternmost_longitude\" type=\"float\">180.0</att>\n" +
                ...
                "        <att name=\"sensor\">MODIS, " + (withGCOM ? "AMSR2, " : "") + "AVHRR, in-situ</att>\n" + // changes
                                                                                                                  // periodically
                "        <att name=\"source\">MODIS_T-JPL, MODIS_A-JPL, " + (withGCOM ? "AMSR2-REMSS, " : "")
    ...
    void testGenerateDatasetsXmlUInt16() throws Throwable {
        ...
        String expected = "<dataset type=\"EDDGridFromDap\" datasetID=\"" + tDatasetID + "\" active=\"true\">\n" +
                ...
                "    <reloadEveryNMinutes>[RELOAD]</reloadEveryNMinutes>\n" + // changes
                "    <!-- sourceAttributes>\n" +
                "        <att name=\"_lastModified\">[DATE]T[TIME].000Z</att>\n" + // 2020-08-20 this change by 5
                                                                                   // seconds. why?
                                                                                   // few seconds changes often!
                                                                                   // 2021-11-16 was
                                                                                   // 2020-09-21! We're going backwards!
                                                                                   // //2022-08-31 now 2020-01-06, then
                                                                                   // 2019-12-17
                "        <att name=\"cdm_data_type\">grid</att>\n" +
                ...
                "        <att name=\"temporal_range\">8-day</att>\n" + // 2020-09-21 6-day?! was and should be 8-day. I
                                                                       // reported
                                                                       // it to podaac
                "        <att name=\"time_coverage_end\">[TCE]</att>\n" +
                "        <att name=\"time_coverage_start\">[TCS]</att>\n" +
        ...
        expected = "Attributes {\n" +
                ...
                "  }\n" +
                "  NC_GLOBAL {\n" +
                "    String _lastModified \"[DATE]T[TIME_VARIES].000Z\";\n" + // 2019-12-17T13:42:11.000Z value from
                                                                              // source
                                                                              // drifts a little each time I run this.
                                                                              // 2021-11-16 time jumped backwards from
                                                                              // 2020-09-21!
                "    String cdm_data_type \"Grid\";\n" +
    ...
    void testUInt16Dap() throws Throwable {
        ...
        try {
            ...
            results = results.replaceAll("(2019\\-12\\-17|2022\\-03\\-23)", // flips between 2019-12-17 and 2022-03-23
                    "[DATE]");
            results = results.replaceAll("\\[DATE\\]T.*.000Z", // date changes sometimes //2021-11-17 was 2020-09-21,
                                                               // now
                                                               // 2019!
                    "[DATE]T[TIME].000Z");
    ...
    void testScale1Offset0() throws Throwable {
        ...
        try {
            eddGrid = (EDDGrid) EDDGridFromDap.oneFromDatasetsXml(null, "testDAPnodcPH2sstd1day"); // 2022-05-23 new
                                                                                                   // THREDDS fail:
    ...
    void testActualRange() throws Throwable {
        ...
        expected =
                ...
                "String axis \"T\";\n" +
                        ...
                        "    Float32 actual_range 27.72, 31.43;\n" + // source actual_range is Float64 values from 1
                                                                     // file and so
                                                                     // changes often! in real life -> null
                        "    Float64 colorBarMaximum 32.0;\n" +
                        "    Float64 colorBarMinimum 0.0;\n" +
                        "    String coordsys \"geographic\";\n" +
                        "    String ioos_category \"Temperature\";\n" +
