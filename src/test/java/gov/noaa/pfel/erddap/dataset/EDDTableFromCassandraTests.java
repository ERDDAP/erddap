...
class EDDTableFromCassandraTests {
  ...
  void testGenerateDatasetsXml(int version) throws Throwable {
    ...
    try {
      ...
      results = EDDTableFromCassandra.generateDatasetsXml(url, props, EDDTableFromCassandra.LIST, "",
          tReloadEveryNMinutes, tInfoUrl, tInstitution,
          tSummary, tTitle, new Attributes());
      expected = "CREATE KEYSPACE bobkeyspace WITH REPLICATION = { 'class' : 'org.apache.cassandra.locator.SimpleStrategy', 'replication_factor': '2' } AND DURABLE_WRITES = true;\n"
          +
      ...
      results = EDDTableFromCassandra.generateDatasetsXml(url, props, keyspace, EDDTableFromCassandra.LIST,
          tReloadEveryNMinutes, tInfoUrl, tInstitution,
          tSummary, tTitle, new Attributes());
      expected = "CREATE KEYSPACE bobkeyspace WITH REPLICATION = { 'class' : 'org.apache.cassandra.locator.SimpleStrategy', 'replication_factor': '2' } AND DURABLE_WRITES = true;\n"
          +
          ...
          (version == 2 ? "    AND speculative_retry = '99.0PERCENTILE'\n"
      ...
      results = EDDTableFromCassandra.generateDatasetsXml(url, props, keyspace, tableName,
          tReloadEveryNMinutes, tInfoUrl, tInstitution,
          tSummary, tTitle, new Attributes());
      expected = "<!-- NOTE! Since Cassandra tables don't have any metadata, you must add metadata\n" +
      ...
      results = EDDTableFromCassandra.generateDatasetsXml(url, props, keyspace, "staticTest",
          tReloadEveryNMinutes, tInfoUrl, tInstitution,
  ...
  void testBasic() throws Throwable {
    ...
    try {
      EDDTableFromCassandra tedd = (EDDTableFromCassandra) EDDTableFromCassandra.oneFromDatasetsXml(null, tDatasetID);
      ...
      expected = "String infoUrl \"http://www.oceannetworks.ca/\";\n" +
      ...
      results = File2.directReadFrom88591File(dir + tName);
      expected = "deviceid,date,sampletime,cascii,cboolean,cbyte,cdecimal,cdouble,cfloat,cint,clong,cmap,cset,cshort,ctext,cvarchar,depth,u,v,w\n"
          +
          ",UTC,UTC,,,,,,,,,,,,,,m,,,\n" +
      ...
      results = File2.directReadFrom88591File(dir + tName);
      expected = "deviceid,sampletime,cmap\n" +
          ",UTC,\n" +
      ...
      tName = tedd.makeNewFileForDapQuery(language, null, null, query,
          dir, tedd.className() + "_subset2", ".csv");
      results = File2.directReadFrom88591File(dir + tName);
      expected = "deviceid,sampletime,ctext\n" +
          ",UTC,\n" +
      ...
      tName = tedd.makeNewFileForDapQuery(language, null, null, query,
          dir, tedd.className() + "_distinct", ".csv");
      results = File2.directReadFrom88591File(dir + tName);
      expected = "deviceid,sampletime,cascii\n" +
          ",UTC,\n" +
  ...
  /**
   * This is like testBasic, but on a dataset that is restricted to 1 device
   * (1001).
   *
   * @throws Throwable if trouble
   */
  ...
  void testStatic() throws Throwable {
    ...
    try {
      ...
      results = File2.directReadFrom88591File(dir + tName);
      expected = "deviceid,date,time,depth,latitude,longitude,u,v\n" +
          ",UTC,UTC,m,degrees_north,degrees_east,,\n" +
          "1001,2014-11-02T00:00:00Z,2014-11-02T01:02:03Z,10.1,34.0,-124.0,-0.41,-0.42\n" +
          "1001,2014-11-02T00:00:00Z,2014-11-02T01:02:03Z,20.1,34.0,-124.0,0.0,0.0\n" +
          "1001,2014-11-02T00:00:00Z,2014-11-02T01:02:03Z,30.1,34.0,-124.0,0.41,0.42\n";
