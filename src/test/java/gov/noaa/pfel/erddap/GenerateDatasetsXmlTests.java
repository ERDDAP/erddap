...
class GenerateDatasetsXmlTests {
  ...
  void testDoItEDDTableFromAsciiFiles() throws Throwable {
    ...
    String sampleFile =
        Path.of(EDDTestDataset.class.getResource("/datasets/test_chars.csv").toURI()).toString();
    ...
    Test.ensureTrue(
        result.contains("<dataset type=\"EDDTableFromAsciiFiles\""), "result=" + result);
  ...
  void testDoItInvalidType() throws Throwable {
    ...
    // Result content comes from the file, which should be empty if no successful generation
    // happened
    // But the log should contain the error
    String logFileName = EDStatic.config.fullLogsDirectory + "GenerateDatasetsXml.log";
    String ra[] = File2.readFromFileUtf8(logFileName);
    Test.ensureTrue(
        ra[1].contains("ERROR: eddType=InvalidType is not an option."), "ra[1]=" + ra[1]);
  }
}
